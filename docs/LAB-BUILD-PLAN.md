...
**Step 13c — Platform serving: full breadth without BYOK.** *(Operator-inserted 2026-08-15; executes here, right after 13a. Lettering is not execution order — 13b still executes late.)*
Work: **guarantee-product core work**, not Lab work — it lands product-agnostic under the additive contract's rule 2, and the Lab inherits it for free through touchpoint 1. Serve orgs from a **platform-owned OpenRouter key** so the frontier can span the whole catalog rather than the one provider a customer brought a key for. The platform fallback already exists (`context.ts:351-380`); the missing pieces are: move the model registry off `prices.json`-on-disk into the database (a `writeFileSync` registry cannot survive a container redeploy); record **who paid** on every serving row (`byok` is computed and consumed nowhere, so platform-paid spend cannot be invoiced); keep **dial honesty** by separating the *catalog* (reachable) from the *frontier* (measured, and the only thing routable); and give the auto-switch a front door (`base_url` + serving key + bound policy + proof, none of which is durably visible today). Billing stays **invoiced** (`--margin-pct` already exists); platform serving is enabled per-org by the operator and **requires** a hard-stop cap.
Out of scope, carried to 13b: payments, credits, abuse controls, self-serve. `POTION_SELF_SERVE` stays unset — platform serving plus open signup would let strangers spend the platform balance (three named gaps: the budget hard stop fails open on a db error; rate limiting is per-API-key with no org or platform ceiling; soft budgets never block).
Done when: a BYOK-less org serves real traffic on the platform key routed from live-provenance evidence; the catalog survives a redeploy; the invoice bills only platform-paid spend at cost + margin; unmeasured models are reachable but never auto-selected; and the auto-switch is visible with its own proof.
