...
| 12 | Adversarial pass on the Lab surface | 2026-08-14 | — | **spec ready** — [step-12-adversarial-pass.md](specs/step-12-adversarial-pass.md): judged by what it CATCHES. §2 gathers all 17 named deferrals in one table (incl. split-token echo across results, sub-shard reassembly, and the act/read classification audit of all 116 catalog tools); four passes ordered structural → targeted → independent → live; §4 fixes the severity rubric and finding budget BEFORE the pass with an anti-negotiation rule (downgrades need a written falsification) and a dry-not-exhausted termination; §5 names seven prior DoD proofs that get RE-EXAMINED rather than trusted, using this build's own history as the prior (3 instrumented-but-inert values, 2 byte-identical claims false-or-narrower — the 5th, that the proof compares only `requestPayload.tools`, is named here for the first time); §7 is the operator sitting written to be followed once (OAuth app steps, **pinned port 3210** so the callback URL is knowable in advance, zero-scope grant, master-key one-liner, env block, what you'll see, revoke + ledger after); §10 binds criticals to Gate C and publishes what the pass does NOT cover (not a third-party audit). **Discharges the Step 10 live-leg binding.** Awaiting build approval |
