...
| 11 | Superpower packaging and catalog | 2026-08-14 | 6ee05c4 | **complete** — [step-11-superpowers.md](specs/step-11-superpowers.md): NEW packages/lab-superpowers — **29 packages** (116 tools: 87 read / 29 act), every one format-complete, least-privilege, and mini-eval-proven at $0. Action classification drives the pore (act fires / read skips / undeclared→act fail-closed), proven per package by a REAL run suspending before its act tool. Authored strings only — the discovery loop reads the package allowlist and the server only for existence, closing the Step 10 residual where server `tools/list` text reached the prompt. TWO honest axes never merged: proof tier (all `fixture-authored`; nothing live-proven — that rides Step 12's bound leg) and connect posture (3 `ready`; unverified packages are STRUCTURALLY unconnectable, `toConnectorDef`→null, OAuth start 404s). Additions: byte-identical hostile-vs-honest context at the loop boundary; classification-diff gate (pore removal needs version+hash together, and regeneration REFUSES to launder one); fixture provenance stamps + staleness. Pre-commit adversarial review: 10 candidates → **3 survivors, all fixed with pins** — server error text reaching context via leg notes, the gate's regeneration hole, and `usage.preamble` charged-but-unwired (now in the system prompt); two falsified spec claims, recorded in §14. Verify unfiltered green (lab-superpowers 145, lab-runtime 62, lab-mcp 37, server 545, workers 151, lab-gen 50, db 191); walkthrough 3.2s/600s at $0 |
| 12 | Adversarial pass on the Lab surface | 2026-08-15 | 4f1cfb9 | **complete at $0, live leg STAGED for the operator** — [step-12-adversarial-pass.md](specs/step-12-adversarial-pass.md) + [findings](../artifacts/step-12-findings.md) + [target register](../artifacts/step-12-targets.json). Four passes ran in order. **10 findings fixed and pinned, 3 of them CRITICAL**, every one a case where a human's approval did not mean what the code did with it: an approval authorized a DIFFERENT action than the one shown (operator sees `publish({body:'first'})`, the run executes `publish({body:'second — never approved'})`); one "yes" re-armed the pore on EVERY later leg because consumption was in-memory while the durable answer survived the leg-cap exit; and a REFUSAL armed the gate identically to an approval (the gate read `answer !== null` and never read the answer). Fixed by binding the authorization to the action's fingerprint, burning the durable answer on use, and requiring affirmative consent. HIGH: server-controlled JSON-RPC error text reached the conversation through `toolError.detail` — the sibling the spec predicted, which FALSIFIES the Step 11 claim as worded (recorded in step-11 §14b); and the transport followed server-chosen redirects, making the hosted-only wall one redirect wide. MEDIUM ×5: T5/T6 closed by a leg-stateful reassembly sentinel with its residual MEASURED (golden corpus now 9 fixtures); replay's self-containment was false for every superpower run; the duplicated secret vocabulary had drifted (now pinned by a divergence test); the grant-absence sweep never checked hex despite promising "any encoding"; the leg-note refresh path carried an untrusted string. **T8 audited all 116 classifications** against vendor semantics — 5 read tools reclassified to `act` fail-closed, 7 packages version-bumped, 2 default scopes dropped, a fictional Salesforce scope and an inverted PagerDuty description corrected, and `scopeLimits` added because the least-privilege test had been passing on a fiction. Every §2 target closes with a TYPED disposition and a meta-test that refuses an untyped close. The independent pass reports **"budget-capped, not dry"** — round 2 still produced confirmed findings, and 5 stalled verification legs left 2 candidates unverified, recorded as unverified rather than absent. Addition 2 (rehearsal against a mock provider) found **four defects in the live-leg script itself** — 3 that would have wasted the sitting, 1 that would have produced a FALSE PASS on the bound cap-trip DoD (the proof called `tool.run()` directly, which writes no step, so the meter never moved; and the authorized 5-call cap was not expressible at all). Verify unfiltered. **Live leg staged, not run**: it needs the operator's hands (§7).
| 13a | Deploy the stack | 2026-08-15 | 5b57902 | **spec ready** — [step-13a-deploy.md](specs/step-13a-deploy.md). Step 13 was SPLIT (operator) into 13a (deploy) + 13b (Gate C flip); execution order **13a → 14 → 15 → 16 → 13b → 17** (14/15 need a running server; nothing before 15 needs a stranger's payment). §1 is a **vendor recommendation, not a survey** — one pick + one runner-up per slot, grounded in code and web-verified pricing (2026-08-15): host = single always-on VM (Hetzner CX22, ~$5; serverless disqualified because the worker is IN-PROCESS — `server.ts:216` — and a `lab:run` leg runs ~24min on the serving event loop, and F18 forbids >1 replica); Postgres = **argues AGAINST the Neon incumbent** (the `/readyz` 2s db-ping is a HARDCODED constant `readiness.ts:12`, not env-tunable, so autosuspend can only be dodged by disabling scale-to-zero at ~$19–76/mo; pgvector is storage-only so no vendor lock-in) → pick an always-on managed PG with pgvector + PITR (Render Basic ~$6–19), runner-up co-located PG on the VM; Redis co-located ($0); object storage none (all durable data in PG — migration 0021); Caddy+Let's Encrypt DNS-only ($0); Sentry Developer + UptimeRobot (both free). Total **~$11–25/mo**. Found two code gaps that are 13a build work: **`POTION_SERVING_URL` is required by `lab:run` (throws, `handlers.ts:3540`) and absent from the prod compose** — without it a deployed harness can't run; and `POTION_PUBLIC_URL` should be set for correct OAuth callback links. §3 operator provisioning checklist (accounts + exact `.env.prod` values). §4 the seven UNEXECUTED container/TLS rehearsal items with pass criteria. §5 the org-budget mid-run kill re-proven on live pricing (`enforceBudgetHardStop`→429→`killed-budget`), ≤$1 ledgered; records the fail-open-on-db-error residual. Auth on, `POTION_SELF_SERVE` unset. Awaiting build approval. |
| 13c | Platform serving: full breadth without BYOK | 2026-08-15 | 6e6f6c2 | **spec ready** — [step-13c-platform-serving.md](specs/step-13c-platform-serving.md). Operator-inserted; **executes right after 13a**. Guarantee-product CORE work under the additive contract's rule 2 (the Lab inherits breadth free via touchpoint 1 — no Lab code changes). **Key finding: platform serving already works** — `context.ts:351-380` returns the platform provider set for an org with zero BYOK keys, built from the four env keys incl. `OPENROUTER_API_KEY`, and compositions are already cross-provider by construction (`strategies/helpers.ts:60-90`). Four real gaps: **(A)** the model registry IS `prices.json` and `research:scan` grows it with `writeFileSync` (`handlers.ts:1255`) — in a container that vanishes on redeploy, and `loadPrices` is boot-only, so the catalog must move to the db (OpenRouter `/models` verified 2026-08-15 to carry per-token pricing + `supported_parameters` incl. tool support + `max_completion_tokens`; ~338 models); **(B)** metering never records WHO PAID — `byok`/`byokProviders` are computed and consumed nowhere, `request_logs.provider` is NULL on serving rows, and `platformCostUsd` is the same SQL as `costUsd`, so platform-paid spend cannot be invoiced (`--margin-pct` already exists); **(C)** dial honesty — catalog (reachable) vs frontier (measured, and the ONLY routable thing); an unmeasured model is visible and never auto-selected, and OpenRouter's `benchmarks` field is explicitly refused as a routing input (that's the incumbent's game); **(D)** the auto-switch has no front door — base_url + serving key appear once, transiently, on `/policy` and are never re-retrievable; no settings page; the home page still says "bring your own provider keys". Also names the **landmine**: `providerMode` is decided ONCE at boot from env-key presence, so setting a live key flips the whole server live and `guardFrontierProvenance` then discards every mock-provenance frontier → traffic silently rides `liveDefaultStrategy` (handled by re-measuring platform frontiers live, per-cluster state recorded). Verification spend ≤$5. **Self-serve explicitly cannot ride this step** — three named gaps go to 13b: budget hard stop fails OPEN on db error (`budgets.ts:78-80`), rate limiting is per-API-key with no org/platform ceiling (`ratelimit.ts:237`), soft budgets never block. Platform serving is per-org, operator-enabled, and REQUIRES a hard-stop cap. Awaiting build approval. |
| 13b | Gate C flip | — | — | not started — **executes after Step 16, before 17.** Carries the original Step 13's Gate C prerequisites verbatim: payments + credits for fuel; abuse controls; guided (self-serve) onboarding; explicit `POTION_SELF_SERVE=1` (the standing decision, never inherited); novice loop measured on strangers. |
