// Track 2 (A3) — hardened-instrument validation leg (2026-08-24).
// Pre-registered: code-gen-hard-v2 and classification-hard-v2 replaced
// saturated instruments; this leg takes fresh salted baselines of the
// champions on the NEW suites. Success = the instrument discriminates
// (top pooled < 0.99, or measurable spread among the top three). If the
// champions ace v2 as well, that negative is published, not buried.
...
const CAP = Number(process.env.LEG_CAP_USD ?? 12);
...
const MODELS = ['or-grok-4.6', 'or-gpt-full', 'or-gemini-flash', 'or-gpt-mini', 'or-sonnet'];
...
const CLUSTERS = ['code-gen', 'classification'];
...
for (const clusterId of CLUSTERS) {
  ...
  for (const [i, salt] of (['a3-val-a', 'a3-val-b'] as const).entries()) {
    const label = `a3-val-${clusterId}-run${i + 1}`;
    console.log(`\n=== ${label} (salt ${salt}) ===`);
    ...
    appendFileSync(`${ART}/ledger.jsonl`, `${JSON.stringify({ at: new Date().toISOString(), week: 'track2-a3', lane: label, spendUsd: spend })}\n`);
