...
const answers = {
  ...
  constraints: [
    ...
    "Verdict semantics: drift detection is ONE-SIDED, collapse only. 'ok' (held) means the canary mean did not fall below the stored interval's lower bound — a cluster scoring ABOVE its stored interval still holds, so never define held as 'inside the interval'. 'drift' means the mean fell below — the routed pick did NOT change and nothing was rerouted, so never say a cluster 'moved to a different model'; 'inconclusive' means the canary could not decide.",
    'A held cluster’s observed mean may sit below its stored quality (within the margin) or above the interval entirely and still hold — never claim that held clusters scored at or above their stored quality, and never claim they all sat inside their intervals. When held clusters scored ABOVE their stored intervals, say so explicitly (count them and name one or two), so a reader is never led to believe held means inside the interval.',
    'A drift flag leads to a full re-measurement, which may RECONFIRM the same pick — never claim a new pick is needed, required or being sought. Say the drifted cluster is flagged for full re-measurement and its stored pick is no longer confirmed.',
  ],
  qualityBar:
