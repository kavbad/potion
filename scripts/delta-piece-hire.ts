...
const answers = {
  ...
  constraints: [
    ...
    'House style: short declarative sentences around the finding, longer ones for qualification; numbers rather than adjectives; explain each technical term in plain words on first use; no em dashes, no headings, no bullet lists, no marketing language, and no superlatives the numbers do not support. Round a figure to the precision the claim needs and use the same rounding the headline and summary use — $44.15, not $44.1479; 404 times, not 403.7 times.',
    'NEVER SAY IT TWICE. Each figure is stated once, in paragraph one. Any LATER paragraph carrying three or more figures that all appeared in the first is REFUSED by a deterministic check and thrown away, however good the sentences are — paragraph three exactly as much as paragraph two. Naming one figure to land a point is fine; restating the comparison is not. A paragraph with nothing to add beyond the numbers should be the shorter thought that is true, not padding.',
  ],
  qualityBar:
