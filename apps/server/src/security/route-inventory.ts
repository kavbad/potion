...
export const ROUTE_INVENTORY: RouteInventoryRow[] = [
  ...
  { method: 'GET', path: '/api/router/generations/:id/evidence', surface: 'api', mutating: false, guard: 'viewer', probeUrl: '/api/router/generations/gen-x/evidence', tenancyClass: 'org-param', resourceParam: ':id', crossOrgProbe: { expect: 'skip', skipReason: "org-scoped getRouterGeneration → a foreign org's id is a uniform 404 (pinned in generations.test.ts)" } },
  { method: 'POST', path: '/api/router/generations/:id/canary', surface: 'api', mutating: true, guard: 'admin', probeUrl: '/api/router/generations/gen-x/canary', probeBody: { rate: 0 }, tenancyClass: 'org-param', resourceParam: ':id', crossOrgProbe: { expect: 'skip', skipReason: "org-scoped getRouterGeneration → a foreign org's id is a uniform 404 (pinned in generations.test.ts)" } },
  ...
  { method: 'GET', path: '/api/floor/feasibility', surface: 'api', mutating: false, guard: 'viewer', tenancyClass: 'self-scoped', crossOrgProbe: { expect: 'skip', skipReason: 'reads the CALLER\'s own bound policy against the org-preferred frontiers (2026-09-16 feasibility-before-commitment) — no cross-org parameter exists' } },
