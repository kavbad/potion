...
import { strategyHash, type Frontier, type FrontierPoint, type Policy } from '@potion/core';
import { resolveOperatingPoint } from '../src/routes/chat.js';

const SINGLE = { type: 'single', model: 'mock-cheap' } as const;
const CASCADE = { type: 'cascade', stages: [{ model: 'mock-cheap', escalateIf: { confidenceBelow: 0.9 } }, { model: 'mock-frontier' }], confidenceMethod: 'self-report' } as const;
...
const frontier = (points: FrontierPoint[]): Frontier => ({ clusterId: 'agentic-tool-use', version: 1, points } as unknown as Frontier);
