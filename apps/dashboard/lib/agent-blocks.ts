...
The model that answered is in a second header, \`x-potion-model\` (a model name, or \`combination:<type>\` for a measured combination — a synthesized program adds its name, \`combination:program:<name>\`). The response body's \`model\` field only echoes the label you sent.
