// /settings/provider-keys — BYOK (SPEC §9 flow 1). Server component renders
// the masked list of stored keys; the form + per-key actions are client
// islands.
//
// MOVED HERE from `/` (SERVING-ROADMAP S1). Bringing your own provider keys
// was the front door, which made BYOK look like the price of entry — it never
// was, and framing it that way also caps what Potion can pick from to
// whatever one provider the customer happened to bring. Platform serving is
// the default path now and lives at `/`; this is the option, for a customer
// who wants their traffic billed to their own provider accounts.
//
...
export default async function ConnectKeysPage() {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-semibold tracking-tight">Bring your own provider keys</h1>
      <p className="mb-10 mt-2 text-sm leading-relaxed text-soft">
        Optional. Potion already serves your traffic on its platform keys — connect your own only
        if you want that provider billed to your account instead. Connected keys take over serving
        for their provider immediately; every other provider stays on platform keys.{' '}
        <span className="text-faint">
          Note that a connected key narrows what Potion can route to for that provider to the
          models your account can reach.
        </span>
      </p>
