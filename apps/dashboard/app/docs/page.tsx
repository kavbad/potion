    ...
    <div className="max-w-3xl space-y-12">
        ...
        <div className="overflow-x-auto border border-[#d9d5cb] bg-[#fbfaf7] px-6 py-2">
          <table className="w-full">
            <tbody>
              ...
              <Row k="underpowered" v={<>Present only when your floor excluded points on <span className="font-medium text-ink">evidence width</span> rather than measured quality — the count of models that scored at or above your bar but whose confidence interval dips below it. Not a verdict on the model: the evidence is too thin to prove your number there. Settings shows the highest provable quality per kind of work before you commit to a floor.</>} />
              <Row k="policy_override" v={<>Present when the request carried <code className="font-mono">x-potion-policy</code>: the name of the policy row that overrode the key&apos;s default for this call.</>} />
              <Row k="retry" v={<>Appears on the <span className="font-medium text-ink">receipt&apos;s</span> trace and in the response&apos;s <code className="font-mono">potion.retry</code>, never in the header, because it is decided after the first attempt: <code className="font-mono">empty_answer</code> when the first answer was empty and the next measured point served; <code className="font-mono">provider_error</code> when the provider failed and the request failed over once.</>} />
            </tbody>
          </table>
        </div>
      </Section>
      ...
      <Section id="compat" title="Streaming and compatibility">
        ...
        <ul className="space-y-2 text-sm leading-relaxed text-soft">
          ...
          <li><span className="font-medium text-ink">Typed routing on the response</span>: every non-streaming answer carries a top-level <code className="font-mono text-xs">potion</code> object — requested vs resolved cluster and policy, <code className="font-mono text-xs">policy_source</code> (<code className="font-mono text-xs">key_default</code> when you sent no override — omitting <code className="font-mono text-xs">x-potion-policy</code> is the convention for &quot;use the policy bound to this key&quot;), the model that answered, <code className="font-mono text-xs">fallback</code> with a <code className="font-mono text-xs">fallback_reason</code> (<code className="font-mono text-xs">policy_infeasible</code> means no measured point met your bar and the best point served; <code className="font-mono text-xs">reasoning_budget</code> means a reasoning model was skipped under your output budget), <code className="font-mono text-xs">cost_usd</code> as metered, <code className="font-mono text-xs">instrument</code> when a tools or vision frontier served, <code className="font-mono text-xs">retry</code> when a second attempt answered, and <code className="font-mono text-xs">provenance</code>. The <code className="font-mono text-xs">x-frontier-trace</code> header stays the source record.</li>
      ...
      <Section id="api" title="API reference">
        ...
        <ul className="border border-[#d9d5cb] bg-[#fbfaf7] px-6 py-2">
          ...
          <Endpoint method="GET / POST" path="/api/alerts" note="Webhook, Slack, or email (mailto:) notifications. Every new org starts with an email rule to its signup address for budget_warning, budget_exceeded, breaker_open, quality_breach, rollback, policy_infeasible (a floor nothing measured can prove), job_failed (a background job failing repeatedly) and provider_auth (a model provider rejecting our credentials, so completions on it fail until someone acts). Subscribe to evidence_ready to hear when a challenger or a workload is measured and waiting on your decision. POST /api/alerts/test sends a test to any target. admin" />
          <Endpoint method="GET" path="/api/floor/feasibility?floor=" note="What a floor would mean per kind of work before you set it: the highest quality any measured model can prove there, and whether the floor clears it." />
          <Endpoint method="GET" path="/api/incumbents/observed" note="Per kind of work, the model labels your own requests named in the last 30 days, ranked by volume — what you use, observed rather than asked." />
        </ul>
      </Section>
