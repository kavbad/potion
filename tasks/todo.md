...
| 2026-08-14 | Step10 LIVE LEG — **PENDING operator hands-on**. Authorized: connector=GitHub read-only (minimum/zero-scope grant), GRANT_RISK_ACCEPTED=2026-08-13, KEY_RISK_ACCEPTED covers the $1 model fuel, caps fuel $1.00 / 5 tool calls fail-closed. The OAuth grant is INHERENTLY INTERACTIVE (spec §10 pushback, operator-approved): scripts/step10-live-mcp.ts gates fail-closed on BOTH risk vars + peers-unset + POTION_MASTER_KEY + GITHUB_CLIENT_ID/SECRET, boots the real server, prints the authorization URL for the operator to approve BY HAND in a browser (Claude never authenticates), then runs capped, ledgers before/after, drives the call cap past 5 to prove it trips live, and revokes the grant. Ledger rows (before/after) append here when the operator runs it. | ≤$1.00 fuel | pending | pending |
