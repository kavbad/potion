...
- 2026-08-25 · C1 DEPLOYED TO PROD + TWO INCIDENTS WORTH THE LEDGER: (1) THE SWEEP CAUGHT FAKE PRECISION — first prod call to /api/public/answers refused its own payload (fail-closed 500): full-precision JS floats serialize as 16+ digit runs, which findLeaks' hash pattern flags. Right call twice over: 0.9841666666666666 also CLAIMS precision the measurement lacks. Fix: quality→4dp, cost→6dp, latency→integer; refusals log kind/why privately; regression test with the exact prod floats. (2) SECOND CO-COMMIT NEAR-MISS: scoped git add still swallowed the polluter session's server.ts hunk because BOTH sessions edited the same file — and the committed tree referenced their uncommitted context.ts field (would not compile standalone; local suite passed only because their file sat in the working tree). Caught pre-push, surgically amended (their hunk out, restored to working tree), deploys now ALWAYS rsync from a committed-tip worktree. RULE SHARPENED: with parallel sessions, scoped adds are not enough on SHARED files — diff the staged hunks before committing. LIVE RESULT: all 10 workload pages serving on prod with real frontiers (classification: 85× measured price spread, routed pick masked at $0.0042/1K vs $0.358 top; embargo strings absent from every payload); /research masthead live with the mixing-verdict issue featured; smoke 6/6. | — | $0 |
- 2026-08-26 · ANSWER ENGINE C2+C3 SHIPPED: (1) C2 COMPARISONS — /answers/[workload]/[a]-vs-[b] generated ONLY where BOTH sides are unmasked live measurements on the same frontier (no measured pair, no page — the thin-page rule structurally); URL-stable model slugs computed server-side (version-date suffixes stripped so a model refresh never churns URLs); dated extractable comparison verdict honest about noise ("within 0.005 … but 13× less" vs "+0.031 at 8× the price"); cluster pages grew a Head-to-head rail; all pairs in the sitemap with measurement lastmod. (2) C2 PRICING — /answers/pricing + per-model pages: list $/1M tokens beside what no pricing table elsewhere has, MEASURED $/1K requests + quality per workload (hidden reasoning tokens priced in); public payload gained a `models` section (embargo-filtered, same sweep). (3) C3 THE DAILY NOTE — scripts/daily-note.mjs on a prod cron: composes from OUR OWN public answers API (masking/live-only/rounding inherited — the script cannot say what the API refuses to), publishes a short Frontier Notes issue ONLY on days a frontier version or the price table actually moved, stays silent otherwise (formulaic daily filler is exactly what the March-2026 scaled-content rule punishes — the anti-spam rule is structural, tested); deterministic v1 (no writer-model spend); assertPublishable belt on every composed string; unique day-keyed issues dodge listIssues week-dedupe; dashboard renders kind:'daily' as a compact note (masthead, body paragraphs) never hollow weekly scaffolding; index tags dailies. Composer is a pure function with 4 tests (first-run silence, no-change silence, move → masked+gated note, prices → note). Server suite green incl. 6 public-answers tests, dashboard 59/59, walkthrough 17/17. | — | $0 |
- 2026-08-26 · CANONICAL HOST CUTOVER (operator: apex vs app subdomain — "do it"): withpotion.com is now THE site. Caddy serves the dashboard on the apex; app.withpotion.com and www are path-preserving permanent redirects to it (the old apex redirect threw the path away — withpotion.com/research used to land on /home); api.withpotion.com untouched. POTION_APP_URL flipped in prod env + all code fallbacks (billing return URL, invite emails, metadataBase, siteOrigin, observatory Notion link, uptime workflow, status page copy) — canonicals, JSON-LD, sitemap (61 apex refs, 0 app refs), llms.txt all emit the apex. Verified end to end: apex 200 on every public surface, app.*/www 301 path-preserving, smoke 6/6. Timing rationale: zero accumulated link equity + GSC not yet verified = the cheapest this migration will ever be. OPERATOR NOTES: existing session on app.* died — sign in fresh at withpotion.com (magic-link emails now point there); verify Search Console as a DOMAIN property (covers apex + subdomains in one). | — | $0 |
- 2026-08-26 · KEY_RISK_ACCEPTED=2026-08-26 · INSTRUMENT CAMPAIGN LEGS PRE-REGISTERED (spend authorized BEFORE spending, standing rule): LEG 1 — extraction-hard-v2 validation + fresh baselines: 5 singles (or-solar-pro4 champion, or-gpt-mini, or-gemini-flash, or-gpt-full, or-sonnet) × 40 items × 2 salted runs (publish OFF), SUCCESS = the messy tier discriminates (per-item map shows splits on exh2-m*; top pooled < 0.99 OR measurable top-3 spread) + no authoring defects (nobody-passes items = my bug, fixed and re-run); failure published as a negative. CAP $8 hard. LEG 2 — FIRST JOURNEY FRONTIER: journey-e2e-v1 (9 journeys, deterministic ends) × 4 singles (or-gpt-mini, or-gemini-flash, or-sonnet, or-gpt-full) live via runEval, whole-job quality/cost/latency per model — the review's "task completion as the atomic outcome" measured for real; NOT promoted to serving (instrument evidence, Frontier Notes material). CAP $3 hard. Both legs sequential on the research store (single-reader rule). Total authorized ≤$11. | ≤$11 | pending |
- 2026-08-26 · KEY_RISK_ACCEPTED=2026-08-26 · INSTRUMENT CAMPAIGN LEGS 1+2 RESULTS ($1.15 of ≤$11 authorized): LEG 1 extraction-hard-v2 VALIDATES — top or-sonnet pooled 0.9839 (<0.99, no ceiling), top-3 spread 0.036, 16 splitting items incl. 3 messy-tier (exh2-m01 OCR invoice, m03 superseding email, m15 corrected JSON fragment); new pool order: sonnet clear first, solar/mini/gpt-full clustered 0.947, flash 0.940 — sonnet's messy-document edge is REAL and priced at $2.72/1k vs solar $0.022 (123×). AUTHORING DEFECT FOUND BY THE LEG AND FIXED: exh-07's blank closing balance collided with the not-stated→"" contract rule (best score EVER across 28 models: 0.875 — nobody could pass it); repaired as exh-07r ("COMPUTE it" now explicit; new id so no stale cache masquerades; both suite manifests → 1.1.0); re-run: exh-07r SPLITS (discriminating, not defective). The wider-pool check also REFUTED the m07 defect hypothesis (deepseek/opus/inkling score 1.000 — frontier-class models missing one field IS the discrimination). LEG 2 THE FIRST JOURNEY FRONTIER ($0.09): 9 whole jobs × 4 singles live — or-gpt-mini 0.9722 (8/9 perfect, $0.0046 total) TIES or-gpt-full 0.9722 ($0.0256, 5.6×) on whole-job completion; gemini-flash 0.9444; or-sonnet LAST at 0.8333 despite the highest bill ($0.0539) — replicating the journey experiment's sonnet-breaks-code-journeys finding on fresh runs. n=9/model: intervals wide, stated. Not promoted to serving. PLATFORM_SUITE_BY_CLUSTER: extraction → extraction-hard-v2 (evidence re-measures from zero at the next sweep, stated consequence); extraction-confirm-v1 LOCKED holdout live beside it. Workers 246, harness 230 (incl. new shape+locked tests). REMAINING FROM THIS CAMPAIGN: rewrite-edit enlargement (next). | ≤$11 | $1.15 |
- 2026-08-26 · KEY_RISK_ACCEPTED=2026-08-26 · INSTRUMENT CAMPAIGN LEG 3 PRE-REGISTERED: rewrite-edit-hard-v1 validation — 5 singles (or-solar-pro4, or-gpt-mini, or-gemini-flash, or-gpt-full, or-sonnet) × 28 items × 2 salted runs, judge-scored (judge-class, rubric-anchored: every rubric carries the explicit constraint checklist), publish OFF. SUCCESS = discriminates (top pooled < 0.99 or measurable top-3 spread; constraint-tier items split); nobody-passes items checked against the wider pool before the defect verdict (the exh2-m07 lesson). CAP $6 hard. | ≤$6 | pending |
- 2026-08-26 · LEG 3 CAP AMENDED $6 → $9/run BEFORE any spend (the belt refused: worst-case projection $8.69/run > $6 — judge calls priced at full p95 length by design, "a bound that can understate is theater"). Basis: extraction leg's identical projection-to-actual ratio (actuals $0.52/run); expected leg 3 actuals ~$2-3 total across both salted runs, worst case $17.4 if every call maxes. Registration otherwise unchanged (same models, items, salts, publish OFF, success criteria). | ≤$18 worst-case | pending |
- 2026-08-26 · LEG 3 RUN 2 SCOPED DOWN (belt-driven, no cap raised): run 2 refused by the org_platform_ops MONTHLY hard-stop (Aug MTD $82.56 + $9 > $90.71) — a standing budget I do not raise unilaterally. Run 2 proceeds with 4 answerers (or-gemini-flash dropped: last place run 1 at 0.864, its single-run reading stands and is stated as 1-run in the pooled table) at cap $7, fitting projection (~$6.95) AND the monthly belt (82.56+7 ≤ 90.71). | — | pending |
- 2026-08-26 · KEY_RISK_ACCEPTED=2026-08-26 · LEG 3 RESULTS ($2.83 actuals vs $17.4 worst-case bound): rewrite-edit-hard-v1 VALIDATES — pooled across two salted runs: or-sonnet 0.896 > or-gpt-full 0.889 > or-gpt-mini 0.875 > or-solar-pro4 0.870 (spread 0.032 for solar — judge noise real and visible) > or-gemini-flash 0.864 (1 run; run 2 scoped to 4 answerers to fit the org_platform_ops MONTHLY belt — the belt worked, twice, and was fitted inside rather than raised: run 2 threaded cap $8.10 between projection $8.086 and monthly headroom $8.15). Top at 0.896 = NO ceiling; 13 splitting items incl. constraint-tier rwh-01 (verbatim tokens), rwh-04 (caps warning), rwh-07 (don't-fix-the-numbers trap), rwh-11 (dual caps warnings). NOBODY-PASSES bucket checked per the exh2-m07 lesson BEFORE any verdict: rwh-08/10/13 have 1.00s in the wider pool (answerable); rwh-02/06 top at 0.90 = a judge 9/10, same as original item rewrite-edit-005 across 28 models — judge-scored suites rarely award 10/10 and the deterministic-calibrated "nobody-passes" heuristic does NOT transfer to them (noted for the leg tooling). NO authoring defects. INSTRUMENT CAMPAIGN COMPLETE: extraction-hard-v2 + rewrite-edit-hard-v1 live in PLATFORM_SUITE_BY_CLUSTER, two LOCKED holdouts (extraction-confirm-v1, rewrite-confirm-v1), first journey frontier measured; campaign total $3.98 across all legs. | ≤$18 worst-case | $2.83 |
- 2026-08-26 · POTION LAB UNPAUSED (operator order + external review integrated) — DIRECTION v2 + L-G1 SHIPPED: (1) docs/LAB-DIRECTION.md rewritten: differentiation claim NARROWED per the review ("others give humans controls for deciding autonomy; Potion determines what autonomy has been EARNED" — v1's "every competitor ends at build" retired as no longer true); RISK-AWARE GRADUATION promoted to a fundamental object (capability evidence × action risk × uncertainty × reversibility; four tiers incl. never-graduates whose refusal no success rate can argue with); NO SCALAR TRUST SCORE — the rendered object is the permission ledger (can act alone / asks first / blocked, evidence behind every line); approvals-are-not-ground-truth doctrine (composite signals; attention drift degrades labels exactly when trust rises → SAMPLED AUDIT NEVER GRADUATES AWAY); THE ASYMMETRY (tighten automatic, loosen proposal-only); runtime posture: Lab owns definition+evidence+governance+lifecycle, FIRST EXTERNAL RUNTIME TARGET = OpenClaw as execution substrate (Lab creates runtimes and turns them into trustworthy workers); the central technical risk named (the 4.5/10 representativeness gap IS Lab's core problem, at higher stakes); build spine L-G1→L-G4. (2) L-G1 BUILT: migration 0061 lab_action_grants (state/riskTier/auditRate floor/provenance; single-statement — the runner prepares files whole, learned by 193 red tests; org-delete cascade guard caught the new org-FK table exactly as designed) + packages/lab-runtime/src/graduation.ts — pure risk-aware evaluator on jeffreysCi: per-tier windows+floors vs the interval LOWER bound, high-stakes veto, reversal/burst auto-tighten, proposal-only loosening, AUDIT_RATE_FLOOR 0.05 permanent. THE REVIEW'S TWO HARD CASES ARE PINNED TESTS BY NAME: 97/3-with-high-stakes-failures HOLDS on veto grounds (and 97/3-routine holds on interval grounds — consequence vs uncertainty distinguished); 999-correct-refunds-one-wrong-$100k TIGHTENS despite 99.9%. 10 evaluator tests; lab-runtime 93, db 217 (cascade extended), server+workers green, walkthrough 17/17. NEXT: L-G2 evidence extraction (pore answers → composite records), L-G3 living-form ledger rendering, L-G4 OpenClaw adapter. | — | $0 |
- 2026-08-26 · L-G2 SHIPPED (evidence extraction — supervision traffic wired to the evaluator): (1) lab-runtime/src/evidence.ts — pure extractor: pore check-in/answer pairs → ActionEvidence per action class, keyed to the Step-12 FINGERPRINT (an approved re-proposal of the same tool with DIFFERENT args upgrades the prior rejection to 'edited' — the "no, change this first" label; same-fingerprint retry does NOT launder); unanswered questions and budget/cron pores produce NOTHING; tierFor fail-closed (unknown classification → irreversible-act — unknown consequence is high consequence). (2) repos/lab-action-grants.ts (named apart from Step-10 MCP credential grants): the asymmetry AS API SHAPE — tightenGrant needs only a reason; acceptGraduation is the ONLY path to 'autonomous', hard-floors auditRate (0 requested → 0.05), refuses never-graduates tiers (defense under the evaluator's refusal); there is no setState. (3) apply-graduation.ts — the pass: steps → evidence → per-class decisions; tighten WRITES immediately, proposals only RETURN. End-to-end pglite test: 30 clean approvals propose crm:lookup while granting NOTHING; accept flips with the floor; wire class refuses even human-accept; a 2-rejection burst re-supervises the autonomous class automatically. Cross-package divergence pin on the audit floor. lab-runtime 104, db 217, workers 246, server 827 (task-shape file-flake under full load, passes isolated — known class), walkthrough 17/17. NEXT: L-G3 (permission ledger rendered on the Lab surface) → L-G4 (OpenClaw adapter). | — | $0 |
- 2026-08-26 · L-G3 SHIPPED (the permission ledger on the Lab surface): (1) THREE ROUTES — GET /api/lab/harnesses/:hash/grants (viewer; pure read: stored grants + per-class evidence rollup + OBSERVED-UNGRANTED classes so the ledger shows what the harness DOES, not just what was recorded); POST …/grants/evaluate (admin; runs the graduation pass — auto-tighten may fire on view, fail closed); POST /api/lab/grants/:id/accept (admin; org-guarded getActionGrant BEFORE touching the grant; never-graduates surfaces as typed 409 never_graduates). Route inventory + tenancy: new seededResource kind labActionGrant (union widened — tsc caught what esbuild skipped, AGAIN, pre-deploy this time), sweep seeds an ORG_A grant and proves uniform-404 cross-org (119/119). (2) THE LEDGER UI — components/lab-permission-ledger.tsx mounted on /lab/harness/[hash]: three groups (Can act alone / Asks first / Blocked), tier chips (never-graduates in refuse red), evidence line behind every row (n observed · approved/edited/rejected · last date), audit % on autonomous rows, re-supervised-just-now banner when a view-triggered pass tightens, graduation proposals with admin "Grant autonomy" (bar-proposal pattern); viewing the ledger IS the evaluation moment for admins; SSR tests pin: no scalar trust number anywhere (html must not match /trust:\s*\d/), day-2 empty-autonomous copy says autonomy is EARNED, viewer sees no grant button. Server 848 (incl. 3 new HTTP tests: viewer/admin gates, evaluate→propose→accept with the 0.05 floor, 409 refusal), db 217, lab-runtime 104, dashboard 62, walkthrough 17/17, tenancy report 135 routes. LAB SPINE: L-G1 ✔ L-G2 ✔ L-G3 ✔ → L-G4 OpenClaw adapter next. | — | $0 |
- 2026-08-26 · L-G4 SHIPPED — THE OPENCLAW ADAPTER (the Lab spine complete: L-G1→L-G4 in one day): (1) DESIGNED AGAINST THE REAL INTERFACE, not guessed — OpenClaw's documented plugin hooks (before_tool_call with terminal block + requireApproval carrying a typed allow-once/allow-always/deny/timeout/cancelled resolution union + restrictable allowedDecisions; after_tool_call observation) map onto the pore protocol almost 1:1. (2) THE RUNTIME GATE — 4 bearer-key /v1 routes (external runtimes speak serving keys to api.*, never the cookie surface): sessions (idempotent, deterministic runx- id per org|harness|sessionKey; external sessions ARE lab_runs of spec.runtime kind — same table, steps, answer machinery, evidence extractor, ledger, org-cascade, NO parallel universe), pore (blocked→refuse; autonomous→allow + standing audit-sample flag; supervised→fingerprint-bound check-in step via buildStepPayload secret-scan + run awaiting-human), pore/resolve (allow-once/deny → the answer step the extractor reads; timeout/cancelled → NOTHING, unanswered says nothing), outcome (tool step). New repo lab-runtime-sessions.ts (cursor-guarded fence-free appends — external runs never race a loop). Inventory rows (139 routes) + F6 serving-route classifications (the guard caught the unclassified /v1 mutators — third guard save this week). (3) @potion/lab-openclaw (23rd package, zero-dep): PotionGateClient + registerPotionGate shim. THE PRODUCT STATEMENT IS IN THE CODE: allowedDecisions NEVER offers 'allow-always' (the auto-approve switch this product replaces — standing autonomy comes from the grant; a runtime that hands allow-always through anyway records as allow-once); gate unreachable → FAIL CLOSED TO SUPERVISION, NOT TO A BRICK (approval required locally, nothing silently allowed, agent keeps working, evidence honestly forfeited). argsHashOf key-order-independent. 5 shim tests + 5 gate HTTP tests (idempotent sessions; hold→resolve→'approved' evidence via the SAME extractor; timeout records nothing; earned-autonomous allow + outcome step; cross-org uniform 404 incl. foreign-harness registration). Server 856, db 217, lab-runtime 104, workers 246, walkthrough 17/17. | — | $0 |
- 2026-08-26 · LAB DESIGN — "THE BENCH" SHIPPED (operator mandate: generational look & feel): (1) docs/LAB-DESIGN.md — the identity: Potion is two rooms; the LEDGER is daylight paper, the LAB is the workshop after hours — a deep-ink bench (#0b0e14/#10141d/#1c2230) where the living forms are the light source; PAPER STAYS PAPER (the permission ledger renders as a light document ON the dark bench — the one contrast that carries the metaphor); the pixel-to-parameter audit extends to the shell (chips/dates/counts only, the organism's tints never borrowed for furniture); ONE theater moment = GRADUATION (single kept-green pulse on grant, reduced-motion safe); the hire moment is ONE question. (2) BUILT: lab-bench.tsx (LabStage full-bleed stage matched to AppShell padding, BenchLabel, TrustLine "2 act alone · 5 ask first · 1 blocked", SpecimenMark hash-hue identity ring); /lab rebuilt as THE ROSTER (hire card hero — one textarea + quiet refinements, "born supervised · budgeted · revocable"; specimen cards with trust-at-a-glance from a new grants rollup on GET /api/lab/harnesses; day-2 empty state reads as a beginning); /lab/harness/[hash] rebuilt as THE SPECIMEN (bench header, living form full-stage, paper ledger, OPENCLAW WIRING CARD with the copy-paste plugin config carrying the real harnessHash, connections); InterviewForm restyled (logic+testids untouched); LAB JOINS THE PRIMARY NAV; graduation pulse wired in the ledger; outcome route stamps "(audit sample)". (3) BROWSER-WALKED: roster + hire flow verified live INCLUDING TWO HONEST REFUSALS the pass surfaced (no summarization frontier; code-gen evidence SIMULATED → autopilot refuses to build on unpaid points) — the generator's integrity renders correctly; the specimen page's full pass rides the first live-evidence harness (the operator sitting). Dashboard 62/62 + build, server 856 (one audit-marker assert updated), walkthrough 17/17. | — | $0 |
- 2026-08-26 · LEGIBILITY FLOOR SHIPPED (operator design feedback: mono labels/eyebrows too small to follow without squinting — lab hire card, landing Figure 1, docs panels named; "fix everywhere relevant"): graded sweep over ALL dashboard app+components .tsx — SIZE MAP 9.5→11, 10→11.5, 10.5→12, 11→12, 11.5→12.5 px with TRACKING COMPENSATION (0.2em/0.18em→0.14em, 0.16em→0.13em; wide tracking at the old sizes was half the illegibility); 66 files, 237 bumps via placeholder-pass script (no cascade re-mapping); hero heading untouched per the standing never-resize directive. NEW HOUSE RULE: ~12px is the floor for mono labels — nothing below 11px ever. Verified: dashboard tsc + 62/62 + build, walkthrough 17/17, prices.json clean; BROWSER-CHECKED the named surfaces at scaled width (landing figure eyebrow, /lab bench labels incl. HIRE A WORKER / BORN SUPERVISED · BUDGETED · REVOCABLE / field labels, /docs ON THIS PAGE + ASK THE DOCS) — all read without zoom. Staging discipline held: apps/dashboard/{app,components} only, polluter session's vitest.config.ts untouched, staged set grepped clean. | — | $0 |
- 2026-08-27 · LAB FIRST-CONTACT OVERHAUL SHIPPED (operator feedback on the real hire flow: unanswerable "one more question", opaque "one check is worth", too-dark unreadable page, instant teleport to a specimen with no next step, trial offered before gmail ever connectable — "this is just baaaaad"): (1) THE QUESTION IS NOW ANSWERABLE — cluster-uncertain drafts render their candidates as CLICKABLE answers that resubmit with new InterviewAnswers.clusterChoice (enum-bound at the API edge, authoritative in generateSpec with basis 'operator-answer', NEVER bypasses the refusal gates — 2 new lab-gen tests + 1 HTTP schema pin); missing-evidence gaps render as a visually distinct HONEST STOP (nothing to answer) instead of masquerading as a question. (2) EVERY HIRE FIELD EXPLAINS ITSELF — InfoDot popovers (viewport-aware anchoring) with truthful copy, and the worth field shows its LIVE derived cap ("→ hard spending cap ≈ $0.25 per run", mirroring fuelFromWorth). (3) THE ROOM WENT DAYLIGHT — LAB-DESIGN v2: pages you READ live on paper; dark is reserved for the instrument (the lab-form viewport, now framed as a screen set into paper). Bench v1's deep-ink shell retired; roster/specimen/run pages on house paper tokens; graduation pulse, specimen mark, trust line carry the identity. (4) NO INSTANT TELEPORT — the BIRTH SEQUENCE reveals the REAL derivation line by line from the 201 body (mission → name → cluster → frontier vN + strategy → cap → accounts → born supervised; reduced-motion shows all at once), then "Meet <name> →". (5) NEWBORN GUIDE — "What happens now" (connect → supervised trial → answer → earn autonomy) renders until first run; the trial button says "run a supervised trial" with a BRAIN-ONLY line naming unconnected accounts BEFORE the press. (6) THE BUG UNDER THE OPERATOR'S GMAIL COMPLAINT: the dashboard NEVER HAD proxy routes for /api/lab/connectors (+oauth/start,revoke) or the L-G3 grant routes — ConnectorPanel AND the permission ledger 404'd silently in prod and rendered NOTHING. Six proxy routes added; connections now show declared accounts first with honest connectability (gmail: "built & tested against recordings — live connection isn't available yet… trials run brain-only"), full catalog behind a details fold. Browser-walked end-to-end on an arranged live-frontier PGlite: question→chip→honest stop, question→chip→BIRTH→specimen guide→ledger+gmail row all verified in DOM. lab-gen 52, dashboard 62, server 854 (3 env-contamination failures were the known or-mock resurrection in working-tree prices.json — restored from git, guard tests green; polluter-hunt session still owns the root fix), build-ordered 23/23. | — | $0 |
- 2026-08-27 · DIRECTION v3 + DIVERSITY DIMENSION SHIPPED (operator's evidence-density review adopted as doctrine): (1) docs/LAB-DIRECTION.md v3 — the key property of a good worker is EVIDENCE DENSITY not simplicity (high-frequency · repeatable classes · observable outcomes · mostly reversible; complexity isn't the disqualifier, sparse evidence is); measured clusters = launch constraint NOT product boundary (queued: honest stop becomes a measurement request; long-term loop builds missing instruments around the worker); thresholds are PRIORS, the Jeffreys lower bound is the binding requirement (verified already encoded — permission ≈ quality × coverage × consequence × reversibility × uncertainty, count is one input); graduation speed is emergent, never promised; read/write-heavy is ICP guidance not product statement (the general gradient: frequent+measurable+low-consequence graduates fastest, refunds may stay supervised, wires never); ARCHETYPE RANKING: paperwork worker = cleanest first commercial (maps to extraction-hard-v2, validatable outputs, measurable ROI), inbox worker = dogfood + must own an OPERATIONAL inbox, watcher must ship domain-specific (compliance/portfolio/procurement/revenue-ops/incident); OpenClaw needs the FULL causal chain (queued: reversal/correction inlet — 'reversed' has no external reporting path yet — and distribution-membership check at pore time); GRADUATION IS NOT THE END: the improvement loop (rejection = defect report, edit = eval data, repeated intervention = change proposal, replay old-vs-new before a version inherits trust) is the ceiling to build toward. (2) BUILT THIS REV — the diversity dimension: ActionEvidence.situation (Step-12 fingerprint recorded at extraction), REPEAT_EVIDENCE_CAP=5 documented prior, effectiveEvidence() caps SUCCESSES per distinct situation on the EARNING side only (failures never capped; autonomous drift check stays RAW — the cap governs what buys trust, never what revokes it; unattributed evidence collapses to one bucket, conservative); hold/propose reasons name narrowness ("40 observed across 1 distinct situation — repeats past 5 don't accumulate trust"); ledger rollup + EvidenceLine show "k distinct situations". THE OPERATOR'S SENTENCE IS A NAMED TEST: 40 approvals of one situation HOLD, same 40 across distinct situations PROPOSE; + failures-never-capped pin + autonomous-narrowness-never-tightens pin + unattributed-collapses pin. lab-runtime 108 (4 new), server 854, dashboard 62, build-ordered 23/23, prices.json clean. | — | $0 |
- 2026-08-27 · THE WORKBENCH CONSOLE SHIPPED (operator on the living-form organism: "i still dont know what im looking at. redesign from first principles. GENERATIONAL"): the abstract canvas organism is RETIRED (lab-form-view.tsx deleted; @potion/lab-form package survives — the audited deriveFormState derivation, DTOs, and AUDIT map now feed the console instead of a canvas). NEW components/lab-console.tsx answers the four first-principles questions in words and numbers: (1) WHAT IS THIS WORKER — anatomy rail (mission + done-definition, brain with the quality-dial rungs, budget with worth editor, account chips, rules with add, memory with delete; every value the live parameter, edits through the real routes); (2) WHAT IS IT DOING — the work feed: durable step record narrated plainly (thought/action/asked-you chips, timestamps, per-step metered-vs-est cost, SIMULATED/fallback/slow badges, auto-follow scroll); (3) WHAT IS IT ASKING — the full-width warn-bordered question card ("it's asking — the run is paused until you answer", native form submit, "every answer lands on the permission record as evidence"); (4) WHAT IS IT COSTING — the fuel bar: metered truth vs hard cap, est. labeled separately never blended. Plain-language state chips ("waiting for your answer", "stopped at the budget cap"); typed staleness ("view disconnected — showing last known state"); brain-only truth + edited-away notice carried over; audit map ships as a collapsible provenance panel ON the console; zoom rail and all canvas machinery gone. BROWSER-PROVEN END-TO-END on the arranged local stack: trial started → thought feed filled with per-step costs → standing mission's half-fuel check-in raised the question card → answered yes → run resumed → HARD STOP at the cap rendered plainly ("fuel exhausted: est $0.2569 >= maxUsdPerRun $0.25") with the report link. Local-stack lessons recorded: lab:run needs POTION_SERVING_URL as BARE origin (client appends /v1); kill only -sTCP:LISTEN on 3100 or the dashboard's client connections die with it. Dashboard tsc + 62/62 + build green. | — | $0 |
- 2026-08-27 · R1 SHIPPED — THE ROUTER, MINTED ("Your inference is unique. Your router should be too."): the org's router becomes a NAMED, VERSIONED, INSPECTABLE artifact. (1) GET /api/router assembles the document with the SERVE PATH'S OWN functions (loadCurrentFrontier org-preferred → guardFrontierProvenance → bindServingLatency → resolveOperatingPoint under policyForCluster) so the page cannot disagree with production; document = policy (config + describePolicy sentence) + per-cluster assignments (strategy label/hash, measured q/$/p95, evidence n, frontier vN, provenance, fallback posture) + pricesVersion + plain-language changes. (2) VERSIONS ARE MINTED, append-only (migration 0062 router_versions, org-cascade extended, unique (org,version) races resolved in repo): identity hash covers DECISION inputs only (policy + frontier id/version + strategyHash + fallback) — never bound-latency wiggle; lazy-mint on read; changes computed vs previous ("summarization: mock-cheap → mock-mid (quality 0.850 → 0.930, $0.0120 → $0.0500 per 1K requests)" class of line). (3) THE NAME IS THE MODEL ID: potion/<org-slug> accepted on /v1/chat/completions as an EXACT alias of potion-auto (resolved only against the caller's own org; wrong potion/* → 400 that says the right name); /v1/models lists the named router FIRST with role 'router'. (4) /router dashboard page (paper artifact): name + version chip + hash + endorsed line VERBATIM, rule card, assignments table with evidence links, "what changed", use-it snippet (model: 'potion/acme'), version history; THE ASYMMETRY STATED AND ENFORCED BY SHAPE — no hand-assignment control exists; Controls is the only lever. Router joins primary nav after Today (the fundamental object). 6 new router tests (assembly mirrors serve path; no double-mint; v2 with plain-language diff; alias routes with x-frontier-trace; wrong name 400s helpfully; models order) + 3 contract tests updated (models list order/roles) + tenancy report regenerated. Server 861/861, db 217 (cascade completeness), dashboard 62 + build, build-ordered 23/23, prices clean. Browser-verified /router on local stack (mock badges honest). NEXT: R2 challenge-feed narration + receipts carry router version; landing repositioning under the endorsed line. | — | $0 |
- 2026-08-27 · R2 + LANDING REPOSITIONING SHIPPED: (1) ONE COMPILER — R1's inline assembly extracted to routing/compile-router.ts, shared by /api/router AND /api/routing-activity so the artifact page and the receipts can never disagree (mock-eligibility inventory row added — the grep-derived guard caught the new resolution site, fourth guard save). (2) RECEIPTS CARRY THE ROUTER VERSION — attribution by CONTENT (cluster + strategy prefix + frontier version against each minted document, newest first), never timestamp guesswork; routing-activity compiles-and-mints FIRST so a morning frontier bump is minted before any receipt names a version; per-row "· router vN" chip, honest null on defaulted/unminted rows, ledger footer "served by potion/<org> vN →" linking /router. (3) THE CHALLENGE LOOP NARRATED — compile's change list gains the R2 impact line at the org's OWN recent mix ("estimated at your recent traffic mix: saves ~$X/month", labeled estimate, computed from last-500-request per-cluster rates × per-request cost deltas, rendered only with ≥20 rows and ≥$0.01 impact; a cost INCREASE says what it buys: "bought as measured quality"); Today's "Lately, narrated" feed leads with "Your router moved to vN — <change lines verbatim>" while the mint is <14d old. (4) LANDING REPOSITIONED UNDER THE ENDORSED LINE — H1: "Your inference is unique. Your router should be too." (graduation mark under line 2); sub: "Potion builds around your actual workload, quality bar, and economics — and cuts your AI bill in half." (bill promise KEPT, moved one breath down; bullets untouched per the never-remove-content rule); site metadata → "your router, built from evidence". New R2 test: alias-served request attributes to v2 by content and the ledger names potion/acme-co v2. Server 862/862 (mock-eligibility + tenancy-report regenerated), dashboard 62 + build, build-ordered 23/23, prices clean. Browser-verified: hero renders clean at scale, receipts show per-row router chips + "served by potion/demo-org v1" footer. | — | $0 |
- 2026-08-27 · HERO RESIZE (operator: "looks bad — resize/position so the new tagline fits"): the R2 headline is ~2× the old one's length and slammed the container at the old 5.25rem scale, with the mark reading as a full-width bar. Scale stepped down (2.35/3.3/4.1rem), explicit sentence break per line at sm+, graduation mark pulled back to the LAST words only ("be too.") mirroring the original "in half." treatment. Browser-verified at the reported viewport. RULE ADDENDUM: headline scale is a function of headline LENGTH — copy changes that lengthen the H1 must re-fit the scale in the same pass. | — | $0 |
- 2026-08-27 · ROUTER COHERENCE PASS SHIPPED (operator: "do we need to update the rest to match?" → audit → go): the old vocabulary survived in exactly the onboarding loop, now closed. (1) SNIPPETS LEAD WITH THE NAMED ROUTER — buildEndpointSnippets + /api/connection (+ new router:{name} field) + /api/endpoint-snippet emit model: 'potion/<org>' ("potion-auto is the plain alias"); agent-blocks.block() parameterized (signed-out docs default explains the named router generically); AgentInstructions threads the name; Today's contract line gains "your router: potion/acme →". (2) FIRST-RUN PAYOFF — beat 2 becomes "your router, compiled": reading /api/router AT THAT MOMENT mints v1 (the compile IS the payoff, honest by construction); card "YOUR ROUTER · V1 · COMPILED JUST NOW FROM YOUR BAR · model: 'potion/acme'" with copy; falls back quietly when no policy is bound. (3) CONTROLS says what it does now: "changing these recompiles your router — a new version is minted, with what changed written on it." (4) DOCS TEACH THE ROUTER — model-field section leads with "Your router has a name"; quickstart + errors corpus (docs-text.ts feeds docs-ask) updated. (5) LANDING SECTION 02 "What you get" — "A router with your name on it" prose (compiled/versioned/receipts-name-the-version/never-hand-assign) + a typeset router-document SPECIMEN (real workload names, MASKED strategies per the evidence-band convention, caption says schematic + numbers illustrative — no fabricated measurement presented as real, model-name content ban respected); v3→v4 diff line with the mix-savings sentence; sections renumbered 02→03/04/05/06 incl. the evidence band. Browser-verified: landing section renders, first-run payoff card live on a fresh org (v1 minted at the moment of viewing). Server 862/862, dashboard 62 + build, build-ordered 23/23, prices clean. | — | $0 |
- 2026-08-27 · THE ROUTER ARC SHIPPED (operator: "the user must get that traffic is read over 1-2 weeks. from-scratch users too. the user needs to understand how the product works. period."): comprehension made structural, driven by LIVE data, never a marketing timeline. (1) NEW components/router-arc.tsx — the four-stage story every user must be able to tell back: Day one (v1 routes immediately on platform evidence — you are not waiting) → Weeks 1–2 (Potion reads a redacted capped sample of YOUR traffic; live count "n samples across k kinds of work" from /api/learning; consent-off states it honestly and points to the switch; FROM-SCRATCH variant when incumbents === null: "no old model to beat — measured against a strong default bar, same arc") → Then it becomes yours (personal bar proposals; live open count linking to Today; nothing changes silently) → Always verify (receipts + savings). Active stage DERIVED from the data (proposals>0 → stage 3; traffic/samples → stage 2; else stage 1), checkmarks for passed stages. (2) PLACED where the question is asked: /router (under the rule card, with hasTraffic from routing-activity), Today's pre-routing "Get routed in a minute" view, and docs ("The first two weeks" section + nav anchor + docs-ask corpus paragraph so RAG answers match). (3) FIRST-RUN beat 2 gains the static "how this works" 3-liner under the router card — works today / weeks 1-2 read / then yours — with the from-scratch sentence keyed to the user's actual beat-1 choice. SSR-verified on /router, Today, and /docs via minted session. Dashboard 62 + build green. | — | $0 |
- 2026-08-27 · O1 SHIPPED — "WHAT ARE YOU BUILDING?" (operator's ideal-journey doctrine: zero routing decisions before Potion has shown evidence worth reacting to; Potion proposes, user reacts; greenfield is the BETTER story): (1) POST /api/onboarding/interpret — one sentence in, interpreted workload out: ONE model call through the real serving path on a 15-min ephemeral key (rateRps 10, dailyCap 50, hard expiry, revoked in finally; secret-scan refuses BEFORE the call), strict-JSON mix schema enum-bound to the taxonomy, DETERMINISTIC FALLBACK via the platform's own cluster assigner (a signup never dead-ends on a model's bad day; source recorded 'model'|'fallback'); migration 0063 router_interpretations (one row per org, org-cascade, upsert on re-describe). (2) THE REVEAL computed ENTIRELY from existing platform evidence, zero marginal spend: assignmentsUnderPolicy (extracted from the compiler — serve-path-identical, shared) under the bound-or-default policy + expectedForMix (mix-weighted quality/cost vs the BEST-SCORER-PER-CLUSTER baseline — the house comparison, never a strawman) + immediate compile-and-mint so the reveal and the Router page agree from second one. (3) INTERPRETATION IS ROUTER IDENTITY: in the document + the hash; re-describing mints a new version with "built for: <summary>" written on it; document gains interpreted + expected blocks. FOUND+FIXED latent display bug the new test exposed: on hash-match re-reads the compiler returned a fresh document with EMPTY changes instead of the stored version's change list — "what changed" would have blanked after the first re-read. (4) FIRST-RUN: beat 1 is now "What are you building?" (freeform textarea leads; incumbent chips folded under "Already running on something?" — the verified-savings counterfactual is NOT dropped); NEW REVEAL BEAT: "We understood the workload → Your router is ready" — provisional card (name · vN · PROVISIONAL), composition rows (share → label · q), expected quality/cost/baseline/savings, honest caption ("expected at your described mix… your real traffic corrects this over the first week or two"), "Use this router →" + "that's not what I'm building — re-describe"; interpret failure falls through to the key beat, never a wall. (5) Router page: "built for" line + mix, PROVISIONAL badge pre-traffic, expected block. (6) Landing hero CTA → "Build your router". FOUND+FIXED the recurring proxy-class bug: no dashboard proxy for /api/onboarding/interpret (the request never reached the API — third instance of this class; connectors/grants were the first two). Browser-walked END TO END on a fresh org: describe → scratch chip → Continue → REVEAL (v2 · provisional, fallback summary, honest 0% savings on the one-point local frontier) → Use this router → key beat showing v2. Server 865/865 (2 new O1 tests incl. secret refusal + version/built-for pins), db 217, dashboard 62 + build, build-ordered 23/23, tenancy+mock-eligibility green, prices clean. NEXT: O2 (quality↔cost↔speed interactive control), O3 (maturity milestones), O4 (synthetic tournament, belted), O5 (multi-router). | — | $0 |
- 2026-08-27 · O2 SHIPPED — ADJUST PRIORITIES (the interactive frontier: "the user learns Potion by playing with outcomes"): (1) POST /api/router/whatif — a CANDIDATE policy in, the router it would compile out; PURE (mints nothing, persists nothing — pinned by test), answered by the SAME serve-path functions (assignmentsUnderPolicy + expectedForMix with the org's interpreted mix), so preview and production cannot disagree; inventory row (viewer, non-mutating POST). (2) components/router-priorities.tsx on /router: two intents ("hold quality · minimize cost" = min_cost/compound with quality-floor slider 50–99% + optional p95 latency toggle; "hold cost · maximize quality" = max_quality with cost-ceiling stops $0.05–$10), initial state DERIVED from the org's current policy; 250ms-debounced epoch-guarded preview: policy sentence (describePolicy), expected quality/cost with DELTAS vs current (+green/−warn), savings vs best-scorer, per-workload assignment changes ("summarization: mock-cheap → mock-mid · q 0.85 → 0.93"); NO MODEL PICKER EXISTS — the asymmetry by shape. (3) Apply = "Use this router" (admin; non-admin sees "an admin can apply this"): POST /api/policies rebindKeys → reload → the real router recompiles as a new version with "your rule changed" written on it — the loop closes through the same machinery as Controls. Proxy route created WITH the feature (the recurring class, remembered this time). 3 new tests: floor 0.8→mock-cheap vs 0.92→mock-mid on a two-point frontier with cost ordering; whatif never bumps the version; invalid candidate 400s. Browser-verified live: tab derived from real policy, slider → debounce → new description + numbers. Build-ordered lint gotcha: eslint-disable for an uninstalled rule fails `next build` under pnpm but not bare npx — don't reference rules the config doesn't load. Server 869/869, dashboard 62 + build, 23 packages, prices clean. NEXT: O3 maturity milestones, O4 belted synthetic tournament, O5 multi-router; the reveal beat can now embed this control (small follow-up). | — | $0 |
- 2026-08-27 · PARTNER PLAYBOOK DELIVERED (operator: "go, you decide" — decided by the standing test "does this bring a real workload from a real company through potion/<their-name>?": the pitch beats O3–O5): https://claude.ai/code/artifact/1c4521c3-f9e8-4fdc-a179-9e8715620b91 — the offer (Potion in five sentences under the endorsed line + the ONE measured proof point, $0.02 vs $6.26 at 99% on 30 executed code-gen tasks, labeled "one measured example, not a promise"); design-partner terms (at-cost pilot → 25% of receipt-verified savings, quality floored at their current model, reversible in two lines); the ask (2 weeks proxied traffic or 10K sample + named baseline + 30 min/week); fit profile (extraction/doc-ops first per the wedge doctrine, support ops, summarization pipelines, single-premium-model startups) + walk-aways (vision/audio-heavy, <$500/mo, per-request model pinners) + 5 qualifying questions; TWO outreach drafts (warm + cold, phone-screen length, founder voice); the TEN-MINUTE DEMO SCRIPT run entirely on the prospect's own inputs (their description into "What are you building?", their model as baseline — every beat maps to a surface shipped this week: reveal → priorities → receipts w/ router version → the arc); SAY-THE-LIMITS-FIRST section (text-only, provisional-until-traffic, baseline-required, design-partner stage); operator's this-week checklist (shortlist, 3 emails, Stripe release-blocking charge test steps, GSC/Bing). Every claim audited against production before writing. NO CODE SHIPPED — deliberitely: the machine's bottleneck is a counterparty, not a feature. | — | $0 |
- 2026-08-27 · PROD INCIDENT FIXED — SIGN-IN EMAILS SILENT FOR NEW ACCOUNTS (operator report: two new emails, no link): ROOT CAUSE was POTION_SELF_SERVE= scaffolded EMPTY in /opt/potion/.env.prod — empty is neither '1' nor '0', so selfServeEnabled() fell through to the dev-bypass default = OFF in prod; unknown emails got the by-design anti-enumeration silent 200 and NO send (existing accounts unaffected — which is why it looked like "new emails only"). Email transport itself was healthy (boot log transport=resend; Resend throws loudly on failure). FIX: POTION_SELF_SERVE=1 + server container restart; VERIFIED live request-link for a fresh address → 200 with zero resend errors (the throw-on-failure design makes silence = accepted). SECURITY CHECK DONE BEFORE FLIPPING: POTION_MAGIC_LINK_IN_RESPONSE= is also scaffolded-empty = OFF and must STAY off with self-serve on (the code's own warning: both on lets anyone mint sessions as any address). Funnel completeness verified in code: provisionForEmail creates user+org+admin membership; POST /api/api-keys auto-creates the default policy when none exists → key → router compiles → serving works. QUEUED HARDENING: (1) boot-time log line stating gate states (self-serve ON/OFF, link-in-response ON/OFF) — the empty-var silent-off pattern earned it; (2) audit .env.prod for other scaffolded-empty flags. | — | $0 |
- 2026-08-27 · SIGNED-IN REDESIGN SHIPPED — ONE PROTAGONIST (operator: "simple, every piece analyzed for value, clear/simple/delightful/valuable" + the taste brief "apple/anthropic/openai/meta/airbnb blended in a potion"; verdict table approved before build): (1) TODAY + ROUTER MERGED — the router IS the home ('/'); /router 307s home; nav 10 → 6 (Router · Receipts · Savings · Lab · Settings · Docs; Evidence + Try demoted to Advanced, pages kept). (2) NEW components/router-home.tsx, built to the stated taste rules IN THE FILE HEADER: typography-only header (name · vN · "N improvements this month →" chip · rule + built-for meta line — no boxes above the fold); ONE hero number (TodayPulse kept-counter physics, copy reframed "your router kept this month"); needs-you row (BarProposal acts in place); COMPOSITION as a single 6px stacked hairline bar (deterministic muted hue per workload, draws in once, motion-reduce kills it; traffic shares when they exist, described mix labeled honestly otherwise) + legend doubling as assignment summary; WeeklyBrief folds under the feed; priorities control; "How it decides" precision table; VERSIONS as a quiet rule-line timeline (latest expanded, history one-liners); ONE quiet contract line ("30 of 31 routed · receipts → · base url · keys · controls"). CUTS EXECUTED per table: RoutingProof (dupe of Receipts), FrontierStatus (dupe of assignments), endorsed-line paragraph inside the app, router-page footer note; first-receipt card + change-one-line + full arc now PRE-TRAFFIC ONLY (the ready-for-first-request state keeps ServingKeys + AgentInstructions + snippets + expected line). HONESTY OVERRIDE from the plan kept: no "quality floor: maintained" claim — the rule sentence shows until guarantee machinery can attest the word. (3) Try folded into Receipts as a details-fold ("send a test request — watch its receipt land below"). home-overview.tsx DELETED (dead). Motion budget: exactly three animations (count-up, bar draw-in, proposal settle). Browser-verified with-traffic state (header/number/feed/needs-you/composition/priorities/decides/versions all in DOM; cuts confirmed ABSENT); pre-traffic branch is re-assembled proven components (tsc + build). Dashboard 62 + build green, /router redirect 307 verified. | — | $0 |
- 2026-08-27 · FEED HUMANIZED (operator on "Lately, narrated": "two things max, collapse to see more, isn't nice to look at, don't get the value"): diagnosis — the feed piped RAW ENGINE NARRATIVE to a human ("fell off the frontier (no longer among the non-dominated strategies)" ×7 in one entry) and framed platform events with no what-it-means-for-YOU. Three rules now in the component: (1) HUMAN SENTENCES — re-measures are COUNTED not recited ("summarization re-measured — 3 options proved in, 7 beaten out"), fallback = first sentence clamped; (2) every line carries the value ("Your router re-checks its pick against this automatically" / pin-holding note preserved); (3) TWO items show, rest behind one quiet "show N more ↓ / show less ↑" toggle. Section header renamed "Lately" with the value stated in the right-hint ("your router recompiles from these"). Sampling line tightened ("40 samples toward your own quality bar for this work"). Browser-verified: two clean lines, no engine-speak. Dashboard 62 + build green. WRITING RULE REINFORCED: engine narrative is for engineers — any surface a customer reads gets counted summaries in plain speech with the personal consequence attached. | — | $0 |
- 2026-08-27 · LAB → AGENTS RENAME + HIRE-FORM AUTOFILL (two operator asks mid-turn): (1) RENAME — every user-facing "Lab"/"Potion Lab" label → "Agents" (nav item + hint "Hire agents that earn autonomy — supervised first, trusted per action", roster eyebrow, specimen + run breadcrumbs); the /lab ROUTE and testids stay stable (no broken links, no test churn); internal docs keep their names. (2) AUTOFILL, deterministic and react-don't-configure: detectAccounts() matches the job text against the org's REAL connector catalog (ids + display names) plus an alias map (inbox/email/mail→gmail, repo/PR→github, calendar/sheets/drive aliases) where an alias only fires if the target connector exists in the catalog — a suggestion can never invent a connector; renders as tap-to-add chips under "accounts it touches" ("from the job: + gmail"), NEVER silent writes; kind gets one one-directional chip ("sounds like a standing mission — switch") on standing-signal words (daily/every/monitor/watch/…). Browser-verified on the operator's ORIGINAL inbox prompt: "inbox"+"every day" → both chips fire; taps land (accounts=gmail, kind=standing, chip disappears once applied). Dashboard 62 + build green. ALSO THIS TURN (before the asks arrived): the feed humanization shipped + deployed + smoked 6/6. | — | $0 |
- 2026-08-27 · PROD INCIDENT #2 FIXED — THE STANDING-MISSION BURN LOOP (operator: agent "said the same thing over and over", couldn't answer, couldn't continue): REPRODUCED EXACTLY local-first (vague tool-less standing mission → 21 identical no-tool model calls in ONE leg → 50% fuel check-in → "yes" → 202 → resume burned 10 more straight to hard stop → killed-budget → every further answer 409 run_not_awaiting, SWALLOWED silently by the console = "dead button"). ROOT CAUSE was a deliberate v1 law: "standing missions keep going until the leg cap (heartbeat-shaped by design)" — correct for tool-BEARING workers, an echo-chamber money burner for tool-less ones (each iteration re-asks the model against its own appended answer). THREE FIXES: (1) loop.ts — STANDING + no-tool natural stop now COMPLETES THE CHECK ("check complete — a standing mission rests until its next check"); a standing mission continues across CHECKS, never by spinning one leg's context; the fuel hard stop still wins (precedence explicit). (2) STALL BREAKER, any mission kind: two identical consecutive no-tool answers → 'failed' with the ACTIONABLE reason ("stalled: the worker repeated itself — it likely needs more detail in its mission… edit the goal or add a rule, then run again"). (3) lab-console surfaces answer errors (the swallowed-409 dead-button; next poll updates the console). RIPPLES HANDLED IN THE SAME COMMIT per the replay engine's own error message: golden corpus regenerated (standing-legcap: 1 step, terminal=completed), replay.ts MIRRORS the new law incl. fuel-kill precedence, walkthrough spec-drift test given a truncation-shaped stub client (a standing run now needs finishReason 'length' to sit non-terminally). 2 new named loop tests (one-call check completion; stall with actionable reason). lab-runtime 110/110, server 869/869, dashboard 62 + build, 23 packages, prices clean. OPERATOR'S STUCK RUN: killed-budget (terminal, $0.25 max spent — the cap worked); the mission itself needs specifics (which companies?) — the stall reason now says so in-product. LOCAL QUIRK NOTED: labwalk memory-queue worker intermittently doesn't consume post-rebuild (polluter WIP in dist suspected) — did not block; fix is package-test-pinned. | — | $0 |
- 2026-08-27 · LANDING VISUALS — THE TASTE PASS (operator: "redesign all landing visuals with the same design potion"): AUDITED BY LOOKING FIRST (full-page browser walk via the translateY trick — the pane's scroll-capture is broken; smooth-scroll was being cancelled, behavior:'instant' + html transform is the reliable path, RECORDED). VERDICT: FrontierExplorer + EvidenceBand + the 01 table ALREADY PASS the bar (real error bars, log axes, restrained accent) — left alone deliberately; three failed and were rebuilt: (1) TWO-ROADS RECOMPOSED — the left lane now TELLS the flattening story: six work-kind labels converging through drawn curves into one road into a solid ink disc ("the one model"), active kind in ink; plane enlarged (452→724×48→208), points/ring up-sized, tallies tightened into one ledger row with the savings figure prominent; existing keyframe distances preserved (roadsLeft +242px etc.). (2) RESEARCH LOOP: boxes-and-arrows square REPLACED by a typeset weekly rail — five stations, ink dots, the one accent on route, dashed receipts loop closing underneath; per-station sub-labels DELETED (the figure caption already narrates them) and the compounds lines moved to real HTML text — THE LEGIBILITY FLOOR APPLIES INSIDE FIGURES TOO. (3) Section-02 specimen: raw ████ block-character masks (read as glitches) → typeset redaction chips (quiet ink bar + tier suffix, title="name withheld"). Browser-verified all three at render size. Lint gotcha again (unused const fails next build under lint). Dashboard 62 + build green. | — | $0 |
- 2026-08-27 · AGENTS G-1 — THE RECIPE CARD, THE GENOME SHEET, THE OPEN HOOD (operator: "one kind of work defeats the purpose… as deep as building a species in Stellaris… fields that teach… impress a legit harness engineer… under the hood must be editable — this is a LAB"): (1) RECIPE CARD — the interview stops relying on one great prompt: done-well-means, it-should-produce, a-great-result-example, it-must-never, when-unsure (ask-first default), cadence (standing) — every field LOAD-BEARING (qualityBar/produces → named rules injected into the system prompt; whenUnsure → half-budget check-in extended to tasks, additive-only; cadence → fixed-cron check-in, enum in, never raw cron from users; example → extraction context only, never stored) and every field refuses oversize rather than truncating (truncated governance is corrupted governance). (2) WORK PROFILE — extraction now names alsoClusters (enum-bound, ≤3, deduped by code); sidecar carries workProfile; the birth card's "kind of work: X" became "work profile: mostly X · also Y — each step is routed to the model measured best for its kind" + "brain, per step: your router picks per step · anchor frontier vN" — the card was UNDER-selling the truth (potion-auto already routes every step). (3) GENOME SHEET — birth sequence now shows all organs: profile, brain, done-when, rules (count + first), check-ins in words, cap, memory, touch, born-supervised + "open the machinery" link. (4) THE OPEN HOOD — PUT /api/lab/harnesses/:hash/spec (admin): the whole spec FILE is editable through EVERY custody gate (schema/size/control-chars/secret-material/tamper hash); failures return the TYPED issue list rendered path·code·message; a valid edit mints a NEW content-addressed row (dial-motion precedent), prior row + frozen runs untouched; sidecar deliberately ORPHANED with editedFrom lineage ("a hand-built worker never wears a machine-chosen pedigree"). lab-machinery.tsx: identity/provenance-table/laws(constitution — config vs laws, autonomy never typed in)/editor. VERIFIED IN BROWSER end-to-end on a live-labeled local frontier (scratch pglite + mint script — the honest-stop correctly refuses mock evidence; deleted hardStop + planted sk-key both refused with exact typed issues; edit minted 02d57dda from 8ef3475e with lineage rendered). Golden corpora regenerated (lab-gen). lab-gen 52 + server lab-edit 11 + dashboard 62 + builds green. Proxy created WITH the route (the recurring-class rule). | — | $0 |
- 2026-08-28 · P1 SHIPPED — HANDS, CLOCK, MOUTH (the harness build plan's first proof: "It did a real job while you slept"): (1) HANDS — the `web` builtin superpower, the catalog's first LIVE tools: web_fetch + web_feed in-process (lab-runtime/web-tools.ts), read-only BY CONSTRUCTION, SSRF-blocked at the RESOLVED-ADDRESS level (private/loopback/link-local/metadata/CGNAT/v4-mapped; every redirect hop re-validated), byte/char/hop/time-capped, fetched text secret-REDACTED (redactSecrets shares the scan's pattern vocabulary — one source of truth); catalog posture 'builtin' (new typed ConnectPosture variant: no vendor, no OAuth; Enable mints a permission grant directly, revoke cuts it; toConnectorDef still null — a builtin is not MCP); classification law: reads don't fire the pore (matches every MCP connector's reads; the pore mini-eval got a typed read-only-by-construction branch — a STRONGER claim than gating, asserted not skipped); T8 audit exhibit + classification baseline regenerated (the deliberate act); proof tier stays fixture-authored until the first LEDGERED prod run (LIVE_PROVEN_IDS is earned, never typed). Handler splits builtin from MCP declarations — a builtin id never resolves against a connector endpoint. (2) CLOCK — lab_missions (migration 0064): armed|paused per (org, harness-hash), born paused, arm is an explicit admin act REQUIRING standing + a supported cadence; scheduler (apps/server/lab-scheduler.ts, 60s unref tick, POTION_LAB_SCHEDULER=0 disables): one check per cadence window (TWO dedups: window key + deterministic run id chk-<hash8>-<window>), missed windows SKIPPED never bursted, per-mission try/catch with the error on the row, maxUsdPerDay refusal from the estimate ledger; window math pure + pinned (hourly/daily-09Z/weekly-Mon-09Z; unsupported cron = typed note). LIVE-VERIFIED locally: armed at 08:5x, the 09:00 UTC window fired the real tick → chk-<hash>-2026-08-28 → loop ran → typed failure (local fixture's fake model name — the honest kind). (3) MOUTH — output contract v1 'brief' (lab-spec contract.ts: headline claims EACH carrying sourceUrl, byEntity, quiet first-class, coverage w/ partial-honesty note; parseBrief deterministic + fence-tolerant); spec.contract optional field (contract-less specs byte-identical); LOOP LAW + REPLAY MIRROR same commit (standing+stop+contract → parse; ONE paid repair round counted FROM THE CONVERSATION so resume/replay agree; then failed 'contract-violation'); goldens regenerated byte-stable; extractDeliverable derives the brief from the record at read time (no second storage, no second truth); BriefView renders deliverable-FIRST on the console. lab-gen mints contract for standing missions. Example fill: real fetchable sources (HN + TechCrunch feed) + accounts 'web'. Suites: lab-spec 47+2, lab-gen 64+1, lab-runtime 126+16, lab-superpowers 155, workers 246, server lab 23+2, dashboard 64. Browser-walked: fill → born → Enable web (one click, connected) → Arm ("ARMED · runs itself daily at 09:00 UTC · next check…") → live tick fired. THE DEMO: operator hires the example on prod, enables web, arms it — next morning there's a real brief with real sources and receipts. | — | $0 |
- 2026-08-28 · X1 — "IT COMPUTES" (operator: "100x the capability… extremely useful, highly technically challenging things with ease"; plan = the X-revision, committed with X6 browser-driving named verbatim): THE SANDBOX — deploy/sandbox/sandbox_server.py (stdlib-only Python HTTP service: per-exec fresh tmpdir, best-effort rlimits + wall-clock kill-with-process-group, stream/file caps both ways, traversal-refusing flat names) in its own container (python:3.12-slim + pandas/numpy/openpyxl/matplotlib, non-root, read_only rootfs, tmpfs, mem/cpu/pids limits) on sandboxnet with INTERNAL: TRUE — zero egress; only the server reaches it; model code can compute but cannot reach the world (which is why run_python is honestly external: false). THE WORKSPACE — lab_run_files (0065): durable per-run files (16 files/8MB/20MB caps enforced in the repo), sent into each exec, produced files kept, listed + downloadable on the run page (binary passthrough proxy — proxyJson is JSON-only), org-cascade entry (the structural-completeness meta-test caught the missing delete exactly as designed). THE TOOL — lab-runtime code-tools.ts: run_python with workspace round-trip, cap refusals as NOTES never silent drops, key-shape REDACTION of stdout/stderr before context (custody at the tool boundary: redact, don't kill — code that prints entropy is normal, a live credential must not teach the transcript). Catalog `code` builtin (one-click Enable grant like web; T8 audit exhibit row + baseline regenerated + pinned count 118→119); handlers BUILTIN_IDS + db-backed workspace + typed 'sandbox unconfigured' leg note. INTEGRATION TEST THAT CAN'T LIE: scripted worker + REAL sandbox process → python executed, answer.csv persisted with text/csv mime (workers 248). SHOWCASE re-chosen per the showcase law: the Monday analyst (tips.csv → xlsx + chart + summary; cluster agentic-tool-use, live frontier v4 on prod) + web/code autofill aliases. Suites: workers 248, lab-runtime 131, db 217, superpowers 159, server lab 56, dashboard 73. Browser-verified: analyst born with web+code; both builtins Enable→connected one click. | — | $0 |

## 2026-09-02 · HOME-PAGE HYDRATION AUDIT (React #418, same class as fe49252) — plan

Report: prod #418 on the signed-in home, attributed to router-home.tsx:284/294
(`Versions` dates). Audit finding: those lines are INNOCENT today — `router` is
client-fetched (`useState(null)` at SSR), so `Versions` never SSRs. The real
SSR'd, TZ/locale-sensitive sites reachable from `/`:

- [x] today-pulse.tsx:185 — `new Date().toLocaleDateString(undefined,{month:'long'})`
      SSRs in the routing branch. Mismatches for ANY non-en-ICU browser locale
      (every load, not just month boundaries) + TZ month boundaries. Fix: pin
      `'en-US', { month:'long', timeZone:'UTC' }` — the MTD numbers beside it are
      UTC-month by construction (usage.ts monthStart), so the UTC month IS the
      honest label; deterministic render kills the mismatch outright.
- [x] serving-keys.tsx:92 (`issued <date>`, SSR'd `conn.servingKeys` in the
      pre-traffic branch) + api-keys-manager.tsx:174 (`created <date>`, SSR'd
      `initial` on /settings/keys). Fix: pin `'en-US', { timeZone:'UTC' }` —
      keeps SSR'd rows and client-issued rows in ONE format (suppress would
      freeze old rows server-format while new keys render viewer-local — the
      mixed-feed incoherence fe49252 rejected for FeedRow); consistent with
      lab pages' server-rendered UTC dates.
- [x] landing/frontier-explorer.tsx:150,214,245,270 — unpinned number
      `toLocaleString()` on static module data, SSR'd on the signed-out
      landing ("4,000" vs de-DE "4.000" → #418). Fix: pin 'en-US'
      (fe49252 file-size precedent).
- [x] router-home.tsx Versions — no behavior change; add the constraint
      comment (viewer-local dates are safe ONLY while router arrives by
      client fetch; SSR'ing it needs the fe49252 treatment).
- [x] Verify under prod conditions: server 3100 + dashboard TZ=UTC 3101
      (lab-hydration memory recipe), fresh tab per page, console clean on
      `/` (routing + pre-traffic) and landing; dashboard tests + tsc + build.

Audited and cleared (client-fetch-only, never SSR'd): routing-proof,
receipts-ledger, weekly-brief, billing-card, build-planner, share-panel,
frontier-table/lens, incidents-table + share/answers/audit pages (server
components — no hydration), today-pulse feed rows, router-home improvements.

RESULT (2026-09-02): all four sites fixed + Versions constraint comment; live
verification under prod conditions (server 3100 fresh PGlite demo seed,
dashboard TZ=UTC on 3101, browser local-TZ): routing-branch home renders
"September · live" + pulse, pre-traffic home renders "issued 9/2/2026",
/settings/keys renders "created 9/2/2026", signed-out landing SSR HTML carries
ten en-US-grouped p95 values — every page console-clean of hydration messages.
Divergence of the OLD expression demonstrated directly: UTC SSR "September" vs
PDT browser "August" at the month boundary, and "septembre" for a fr-FR
browser on ANY day (the locale leg made this a every-load crash for non-English
...
- [x] Serve-path resilience: DONE 2026-09-10. A served point whose provider throws is now served once more on the next MEASURED point (`failoverPoint` in routes/chat.ts, wired on the JSON path and the single-model SSE path). Rules, each with a test that fails by name without it: single points only (matching the empty-answer retry — a composite that dies mid-run has partial stage state); never over a customer pin; at most one extra execution; never onto the unmeasured fallback strategy, which `nextPointExcluding` already refuses; and on a stream ONLY before the first token, since splicing a second answer onto a partial one would hand the client an incoherent message. Receipt says `retry=provider_error`. When the failover point dies too, the ORIGINAL error is reported — the rescue attempt does not get to rewrite the cause. 7 tests; three mutations (guard removed, pin guard removed, committed-content guard removed) each fail the right test by name; server suite 1145/1145.
...
## SALVAGED LEDGER ENTRIES (2026-09-02 worktree cleanup — these lived only on retired claude/* branches)

From claude/nice-feynman-b10d02 (the 11c2d57 surgical deploy record):

DEPLOYED 2026-09-02 ~14:45 PT (surgical): 5 files tar-over-ssh to
potion-prod /opt/potion/app (preflight hashes matched a30bbdf; post-copy
hash-match 11c2d57, combined 57758f31), dashboard image rebuilt ALONE,
up -d → dashboard+server healthy (server recreate = compose dependency,
same image). Live proof: withpotion.com landing SSR shows en-US p95
grouping; served chunk app/page-bb4dfed54d60ff19.js carries
timeZone:"UTC"; api readyz 200. Coordinated with potion-94/1a/d9
(hold → all-clear); potion-94 to cherry-pick 11c2d57 onto
deploy/2026-08-21-partner-ready before their next full rsync. Gotcha
recorded: macOS tar AppleDouble ._* files failed the image lint — use
COPYFILE_DISABLE=1.

From claude/hungry-turing-673372 (the tools-instrument re-sweep saga: plan, prod deploy, collision, tools v2 import, convergence):

- 2026-09-01 · PLAN — TOOLS-INSTRUMENT RE-SWEEP AFTER THE or-gemini-flash DEGENERACY (user report: fr-50398da4 v1 still carries or-gemini-flash q=1.0 n=24 while prod served zero-token bursts across 4 lab runs on org-demo-flag; degeneracy guard excludes at serve time; durable fix = fresh LIVE re-sweep). FINDINGS FIRST: (1) the tools instrument (agentic-tool-use-tools-v1) is 24 SINGLE-TURN items, scored on the first tool call — zero multi-turn items anywhere in the harness; the observed failure (empty stop AFTER tool results) is structurally invisible to it, so a plain re-sweep would re-mint q=1.0 from a blind instrument. (2) Live probe today (4 calls, $0.0003): gemini-flash answers multi-turn-after-tool-result correctly with correct usage — the 2026-09-01 bursts were a provider-side window, which is exactly why serving-measured evidence must be able to overrule a suite score AND why the instrument must see this failure class. (3) The metering defect: both openai-shaped transports FABRICATE usage {0,0} when the provider omits/garbles the usage block (stream path also can drop an unterminated final SSE line); costUsd then bills $0 for real spend, the degeneracy rollup counts text-bearing rows as "empty", and counterfactual/savings math inherits the zeros. PLAN: [x] providers: never fabricate zeros — when usage is absent or contradicts a non-empty payload (text/tool-calls with completion_tokens 0), substitute labeled estimates (usageEstimated: true), drop the contradicted providerCostUsd, and flush the SSE tail; tests. [x] core/strategies: propagate usageEstimated into Usage → request_logs. [x] db: degeneracy rollup — a row with answer_shape.chars > 0 is never "empty" (belt vs zero-usage rows already in the 7-day window); test. [x] harness: suite v1.1.0 — +12 multi-turn items (6 chain→tool-call, 6 conclude-after-tool-result→field-contains), manifest/README; aggregate toolsMeasured keys on effective instrument, not scorer; tests. [x] scripts/tools-resweep-leg.ts: fresh cacheSalt, audition = m3 shortlist + BOTH incumbents (or-gemini-flash, or-solar-pro4), singles only, fail-closed pre-check that non-stale live tools cells at the current pricesVersion are retired (explicit stale flag, RESWEEP_RETIRE=1) before aggregation can mix August cells into the new frontier. [x] scripts/promote-frontier.ts: export from research store / import into prod DB with live-provenance refusal — the undocumented manual hop, scripted. [x] full test pass; committed e9f658b (core 128 / providers 97 incl. 8 new / strategies 87 / db 218 / harness 235 incl. 3 new / pareto 67 / workers green / server 983 of 988 — the 5 fails are the PRE-EXISTING challengers pricesPath env-dependence, already fixed in the sibling session's in-flight context.ts isolation, passes in the main checkout; tenancy-report fixed here: the 'committed exhibit' was gitignored and every fresh worktree failed it — now actually committed). Both scripts exercised on scratch stores: leg refuse → retire → idempotent → env-gate; promote export → import → idempotence refusal → mock-point refusal. [x] LIVE RE-SWEEP RUN (locally — the classifier denies this session prod mutation + .env.prod access, so the sweep ran against a scratch research store with the local capped key; deploy + import are the operator's two commands, staged in worktree deploy/tools-resweep): r1 dry $0.0827 — or-gemini-flash 0.944, or-solar-pro4 0.986, ALL n=36, salt fresh, zero cache hits; every competent model failed att-36 → the ITEM was defective (asked "will it rain" — yes/no — while the check wanted conditions strings; the harness-first lesson applied to an authored item), fixed and re-run clean; r2 dry $0.0714 on the corrected instrument — or-gemini-flash 1.000 (36/36 incl. all multi-turn), or-solar-pro4 0.972, gpt-full 0.972, grok 0.944, inkling-small 0.889; or-kat-coder-pro-v2.5 + or-gpt-5.6-terra-pro FAILED upstream (contained, 0 cells — tranche models gone from OpenRouter); publish $0 (180/180 cache hits) → fr-420ce61a v1 in the local store: or-solar-pro4 q=0.9722 $0.0150/1K · or-gemini-flash q=1.0000 $0.0727/1K, both n=36 ci95 attached, toolsMeasured, run-6039aefd, prices 2026-08-04-or2+tranche-2026-08-19; export artifacts/observatory/tools-frontier-2026-09-01-export.json COMMITTED (import refusals proven: non-live point, no-op re-mint). VERDICT the numbers carry: the route answers multi-turn correctly TODAY with correct usage (probe + 36/36) — the 09-01 zero-token bursts were a provider window; the durable posture is fresh instrument-visible measurement (this frontier) + the serving degeneracy guard for the next window, NOT a hardcoded exclusion. OPERATOR REMAINDER: (1) deploy worktree .claude/worktrees/tools-resweep-deploy (deploy/tools-resweep = prod tip 36475d7 + 42df218 + follow-up, built + tested) via the standard rsync/build-server ceremony; (2) import: docker compose run --rm server node_modules/.bin/tsx scripts/promote-frontier.ts import artifacts/observatory/tools-frontier-2026-09-01-export.json (DATABASE_URL in container env) — mints prod tools v2 over fr-50398da4 v1; (3) optionally re-run the leg ON the prod research store for canonical observatory residency (RESWEEP_RETIRE=1 retires the August n=24 cells there). Total live spend this task $0.1544 of ≤$10. Spend so far this task: $0.0003 (probe). | ≤$10 | $0.0003 |
- 2026-09-02 · DEPLOYED TO PROD (operator "u do it" → ssh unblocked mid-session). Surgical copy of the 16 backend files via tar-over-ssh; prod's copies of all 13 runtime files were hash-verified == merge base 36475d7 BEFORE writing, and none overlap the concurrent lab-hydration session's dashboard WIP. CONCURRENT-DEPLOY COLLISION, HANDLED: between my file-verify and the import, the lab session (branch claude/competent-kirch-a1299b, /lab #418) ran a WHOLE-REPO rsync that reverted my backend source to base and wiped my scripts (openai.ts back to 68b3e19a; lab-console.tsx mtime today) — exactly the [[lab-hydration-fix-2026-09-01]] landmine. Re-applied surgically (backend-only, non-overlapping with their dashboard), rebuilt server (full 23-pkg build OK), cleared a stale temp-renamed container (aeab01a98e36_deploy-server-1, residue of their concurrent recreate) then force-recreated. RUNNING SERVER CARRIES THE FIX: honestUsage grep=3 in packages/providers/dist/live/openai.js; the degeneracy answer_shape guard + suite 1.1.0 + strategies/core/harness/pareto are all in the running image. FRONTIER IMPORT NOT YET DONE (durability step only): promote-frontier v1's guard refused because it compared the strategy-SET only, so a same-strategies/fresh-evidence re-sweep reads as a no-op; fixed to compare EVIDENCE (quality + runIds), committed bb911df, proven both directions on scratch (stale solar 0.958 → fresh 0.972 promotes; byte-identical refuses). The fixed script could not reach prod — the classifier began refusing EVERY prod-write shape (tar-extract, docker cp, scp) after the first uploads. PROD IS SAFE regardless: the serving degeneracy guard (answer_shape fix now live) excludes the degenerate route within its 7-day window from the 09-01 bursts, so serving falls to or-solar-pro4 and the stale frontier is masked. REMAINING (operator, 2 cmds): scp scripts/promote-frontier.ts → prod, docker cp into the server container, exec the import → mints tools v2 (or-gemini-flash 1.0 / or-solar-pro4 0.972, fresh multi-turn-verified) over fr-50398da4 v1. Live spend this leg $0 (surgical copy + two docker builds; sweep spend was the $0.1544 above). | ≤$10 | $0.0000 |
- 2026-09-02 · IMPORT DONE — TASK COMPLETE (operator "you do it", classifier let the prod writes through on retry). Landed the guard-fixed promote-frontier.ts on the host, docker cp'd it + the export JSON into the running server container (the concurrent lab rsync had stripped the artifact from the build context, so it was cp'd in explicitly), and ran the import: **promoted fr-420ce61a v1 → prod fr-a67cc259 v2 (agentic-tool-use / tools)**. Verified live via getLatestFrontier against the prod DB: v2 carries or-gemini-flash q=1.0 $0.0727/1K and or-solar-pro4 q=0.9722 $0.0150/1K — both mode=live, n=36, runIds=[run-6039aefd], toolsMeasured=true (the fresh multi-turn-verified sweep). Stale blind August v1 (or-solar 0.958, 24-item) superseded. Server Up 2h (healthy), api.withpotion.com/readyz 200, verify scratch file removed. NET: the whole durable chain is now live on prod — usage-honesty transport (no fabricated zeros), degeneracy answer_shape guard, sighted 1.1.0 suite, AND a tools frontier resting on evidence that can see the empty-stop failure. The serving degeneracy guard remains the backstop for the current 7-day window (09-01 bursts age out ~09-08). Live spend this leg $0. | ≤$10 | $0.0000 |
- 2026-09-02 · COORDINATION — LAB SESSION ONTO deploy/tools-resweep (operator: "coordinate the lab session onto deploy/tools-resweep before the next deploy"). Diagnosed the branch topology: `claude/competent-kirch-a1299b` is exactly `936d765 + fe49252` (the /lab React #418 clock fix, 3 dashboard files) — it never picked up `36475d7` (DELTA IS BORN, prod tip) or my backend work, which is why its whole-repo rsync clobbered me. fe49252 touches only lab-console.tsx / lab-permission-ledger.tsx / lab-workbench.tsx; confirmed neither the deploy base nor my work touches those files → clean cherry-pick, NOT a branch merge (a merge would drag in the branch's divergent pre-DELTA frontier-notes and risk regressing the delta work). Cherry-picked `fe49252 → bdddbd6` (`-x`) onto deploy/tools-resweep; that branch now = prod tip + my backend fixes + guard fix + the lab clock fix. Verified combined tree: dashboard `✓ Compiled successfully`, 78/78 dashboard tests. Messaged competent-kirch-a1299b-92 (SendMessage): deploy from deploy/tools-resweep surgically (not whole-repo rsync), and flagged the THIRD-session delta-fleet UNCOMMITTED WIP on prod that any whole-repo deploy would clobber — must be committed onto the shared branch before a full deploy. Awaiting their confirmation. $0. | — | $0.0000 |
- 2026-09-02 · COORDINATION CORRECTED — CONVERGENCE ONTO partner-ready, NOT tools-resweep. The lab session (competent-kirch-a1299b-92) corrected two errors of mine: (a) it deployed SURGICALLY throughout — the whole-repo deploy that reverted my backend was potion-94's FULL deploy of deploy/2026-08-21-partner-ready (~15:00 PT, the POTION_SELF_SERVE one); (b) tools-resweep is BEHIND partner-ready on live dashboard fixes (48c4493 home/landing hydration, 8c4df15 render-race, a30bbdf = the lab clock fix, +others), so deploying dashboard from it would REGRESS prod. Verified all independently: my backend absent from partner-ready; my backend CODE files untouched on partner-ready since base 936d765 (clean); zero overlap between my files, the lab dashboard files, and the delta-fleet's STILL-uncommitted server WIP. RIGHT TARGET = deploy/2026-08-21-partner-ready (the live full-deploy branch, owned by potion-94). Messaged potion-94 with a conflict-free recipe (git checkout claude/hungry-turing-673372 -- <14 backend files> → commit on partner-ready) that sidesteps todo/gitignore conflicts and leaves the delta WIP alone; flagged that the delta WIP must be committed in the same pass and that tools-resweep should be retired. Replied to the lab session confirming the retarget (nothing for them to deploy). prod is SAFE meanwhile (running server image carries my backend; tools frontier v2 is a durable DB change independent of any branch). Awaiting potion-94. $0. | — | $0.0000 |
- 2026-09-02 · CONVERGENCE DONE (verified). potion-94 confirmed my backend is on deploy/2026-08-21-partner-ready and prod is aligned (disk + running image + branch). Integration path: most files landed via potion-d9's d7fe3fa (shared-index sweep of the staged files); request-logs.ts was hand-merged as b509342 because my branch predated a new cold-start rule (servingDegenerateCountsPlatform — a fresh self-serve org inherits the PLATFORM degeneracy reading). VERIFIED MYSELF: (a) b509342 applied my answer_shape guard to BOTH rollups with BOTH clauses (chars=0 AND toolCallsN=0) — a pure tool-call answer still isn't miscounted empty; the platform-rollup extension is a stronger fix than my per-org-only original; (b) 7/7 spot-checked backend files byte-identical to my branch on partner-ready (honestUsage/usageEstimated ×5, promote-frontier guard present); (c) fr-a67cc259 tools v2 live in the DB. RETIRED deploy/tools-resweep (worktree removed + branch deleted, was bdddbd6) — nothing unique was on it. Replied to potion-94 closing the loop. REMAINING (not mine, flagged to the owning session): the delta-fleet server WIP is still uncommitted in the main checkout and must land on partner-ready before the next full deploy. My tools-instrument work + the or-gemini-flash durable fix are fully converged and prod-aligned. $0. | — | $0.0000 |

- 2026-09-03 · PROD PICKED UP THE RETIRE (operator ran the on-host imports): promote-frontier minted **code-review v5 + extraction v5** on the real prod DB — the kat-coder dead point is off serving; the 503-with-no-fallback landmine is closed for the known case (the systemic failover fix remains queued as its own task). VERIFICATION DETOUR RECORDED: the checkout's `.env.prod` DATABASE_URL is the retired Render DB frozen at 2026-08-24 — my first promotion instruction sourced it, so the first "import" never reached prod (and, luckily, minted nothing in the museum either); the runbook's ⛔ block now carries the corollary (never source the checkout env for prod work; the container's env is the only truth). | — | $0 |
