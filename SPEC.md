...
- `POST /v1/traces` (org-scoped, batch): spans in OTel GenAI convention subset {trace_id, span_id, parent_id, name, model?, input_tokens?, output_tokens?, attributes.gen_ai.*}. Payload conventions (G1.4): `gen_ai.prompt` per user turn (multiple spans = ordered turns), `gen_ai.completion` = the session's FINAL assistant answer (last in ts order wins; becomes the SESSION replay item's reference), `tool.args`/`tool.result` on tool spans (replay context). All payload attrs are PII-redacted at ingest — replay references are redacted text and judges treat placeholders as matching any equivalent value; stored trace_spans(id, org_id, trace_id, span_id, parent_id, name, model, usage jsonb, cost_usd, attrs jsonb, ts). Idempotent on (org_id, trace_id, span_id).
- **Per-call spans (post-capstone item 2, Decision 1)**: one `llm.call` span per text-producing model call — `gen_ai.operation.name: 'llm_call'`, `gen_ai.completion` = that call's own output, per-call `model` + token usage, `potion.step_index`. These preserve the prompt↔completion pairing the session convention discards, and are what STEP-LEVEL item synthesis consumes: each sampled call becomes an eval item whose prompt is the reconstructable context that call saw (prior turns, prior step outputs, tool calls) and whose reference is the call's OWN output — never the session's final answer. Rationale (measured): whole-session replay asks one call to reproduce a ~40-tool-call trajectory; the capstone incumbent scored 0.2000 on its own traffic under it (recorded invalid for agentic workloads). Sessions with llm.call spans synthesize into `-replays-v2` step suites under a deterministic sampling policy (first+last+evenly-spaced steps per session, round-robin cluster fill); sessions without them keep the v1 session-item shape — no flag day.
...
- Cluster agent sessions by: first-user-message embedding (existing embedder) + tool-call graph signature (hash of the **canonical** tool-name sequence — DISTINCT tool names in first-use order, `canonicalToolSequence`); cluster id `agent-<slug>`; feeds the SAME frontier pipeline (eval items synthesized from session replays: redact payloads, keep structure).
  - **G2.8 revision (measured, not theoretical).** This originally hashed the *raw ordered* sequence, which is stable for a production agent with a fixed pipeline but degenerate for a free-form one. Over 48 real coding-agent sessions the raw rule produced **45 distinct signatures — roughly one cluster per session**, which defeats `SUITE_VERIFY_MIN_PAIRS` (5), `RUBRIC_PROBE_MIN_REFERENCED` (3), and the premise that a derived suite has more than one item. The canonical form yields 7 buckets on the same corpus, two of them suite-bearing. First-use *order* is retained (read-then-write ≠ write-then-read); only repetition is collapsed. The cluster NAME and `rubricTemplateFor` also use the canonical form, since after canonicalization it is the property the bucket actually shares.
  - Changing the signature changes existing `agent-*` cluster ids. Agent clusters are derived artifacts recomputed from spans, so re-derivation is the migration; incumbent designations and org frontiers keyed to an old id are orphaned and must be re-designated.
- **Suite certification (post-capstone item 3, Decision 2, migration 0031)**: a derived suite is CERTIFIED for guarantee use only if the org's designated incumbent retains its own baseline (self-retention ≥ 0.9) when FRESH-re-evaluated against it (`suite:certify`, admin-triggered; every outcome — certified / failed / recorded refusal — writes a durable `suite_certifications` row keyed on (org, suite, suiteVersion); re-derivation invalidates by key). An UNCERTIFIED suite may be inspected but backs no contractual claim: suite-verify still measures and durably records verdicts but WITHHOLDS contractual effects (no incidents, no advisory resolution, no auto-restore, no alerts — the withholding is itself recorded); the guarantee report renders no retention number (reason instead); the savings report withholds the cluster's shadow samples and reports the withholding. Review surface: /rubrics alongside rubrics; `mode-mismatch` (per-call metering item) is the gate's negative half.
