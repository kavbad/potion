...
describe('T8 audit exhibit — complete, both directions', () => {
  it('every catalog tool has exactly one verdict, and every verdict names a real tool', () => {
    ...
    // (P1, 2026-08-28) + 1 code builtin (X1) + 3 browser builtins (X6) +
    // run_shell and the 2 git builtins (X7, 2026-08-30) — builtin rows cite
    // the runtime implementation, not a vendor endpoint.
    expect(catalogKeys.length).toBe(125);
  });
