...
// A plain object, checked against the real signatures (the house idiom — see
// lab-runtime's contract-law tests): clientFor hands back a
// ServingClientLike, so the stub's replies stay type-checked against
// ServingResult without a real client pointed at an unroutable host.
function scriptedClient(trace: string, text: string, counter: { calls: number }): ServingClientLike {
  return {
