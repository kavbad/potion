...
- ✅ **Randomized incumbent holdout** (0086): consent-gated (default off, rate ≤5%, slice always visible on Savings), the serve path swaps a randomized slice to the NAMED incumbent (pin outranks; default instrument only; never a mock baseline under live; labeled `;holdout=1` + ledger column; baseline NULL, router_version NULL). The Savings page's "Verified savings" block is the only place the product says verified: measured incumbent mean × routed count vs measured routed spend, seeded-bootstrap CI, the LOWER bound named as the billing basis. NEXT integration: swap the invoice basis to `verifiedSavingsLowerUsd` before charging turns on (the Stripe sitting's counterpart).

### G2 — Customer frontiers ⬜
- ⬜ Org workload discovery (generalize the traces:cluster seed beyond agent traffic) → org-specific measured frontiers (live-swept, provenance-guarded) → router generations with shadow/canary/promote/rollback.
